       ...
       */
        ...
        if(isset($dataArr['domainService']) && count($dataArr['domainService']) > 0) {
            ...
            $dataBillingArr [] = array(
                    ...
                    'vDelStatus'        => '0',
                    'vExistDomainFlag'  => $dataArr['existDomainFlag']
                    );
...
//echopre($dataArr);exit;
        ...
        if($vSubscriptionType=='PAID'){ // Billing auto pay entry only for PAID subscription
            if(count($dataBillingArr) > 0) {
                foreach($dataBillingArr as $itemBill) {
                    $vExistDomainFlag  = 0;
                    $vExistDomainFlag = ($itemBill['vDomain'] == 1)?$itemBill['vExistDomainFlag']:0;
                    $invBillQry = "INSERT INTO ".Admincomponents::$dbObj->tablePrefix."BillingMain SET nBmId=NULL, nUId='".$itemBill['nUId']."',
                            ...
                            dDateStart='".$itemBill['dDateStart']."', dDateStop='".$itemBill['dDateStop']."', dDateNextBill='".$itemBill['dDateNextBill']."', dDatePurchase='".$itemBill['dDatePurchase']."', vPaymentMethod='".$dataArr['vMethod']."', vDelStatus='".$itemBill['vDelStatus']."', vExistDomainFlag='".$vExistDomainFlag."'"; 
   ...
   public static function getUserPalnCount($userId) {
    ...
    public static function logServerMapped($idProductLookUp){
        ...
        $dataArr = Admincomponents::$dbObj->execute($sel);
    ...
    public static function getSpecialsCaptureType($type=NULL){
     ...
     public static function cipherPassword($domainId=NULL){
      ...
      public static function getDistributors(){
         ...
         $sel = "SELECT t.*,
                f1.file_path as vDistributorImagePath
                FROM ".Admincomponents::$dbObj->tablePrefix."distributors t
                LEFT JOIN ".Admincomponents::$dbObj->tablePrefix."files f1 ON t.vDistributorImage = f1.file_id WHERE t.vDistributorStatus = '1'";
...
//echo $sel;
     ...
     public static function getPartners(){
         ...
         $sel = "SELECT t.*,
                f1.file_path as vPartnerImage
                FROM ".Admincomponents::$dbObj->tablePrefix."partners t
                LEFT JOIN ".Admincomponents::$dbObj->tablePrefix."files f1 ON t.vPartnerImage = f1.file_id WHERE t.vPartnerStatus = '1'";
...
//echo $sel;
     ...
     public static function getPaidTemplates($filterArr = array()){
         ...
         $sel = "SELECT t.nTemplateId, t.vTemplateName, t.vDescription, t.nCost, t.vTemplateZipId, t.vHomeScreenshotId, t.vInnerScreenshot1Id, t.vInnerScreenshot2Id,
         ...
         $sel .=' ORDER BY t.nTemplateId ASC';
    ...
    public static function getFreePlanId() {
     ...
     //Function fetch username for cms
    public static function cmsUserNameDisplay($userDetails){
          $content = '--';

          if(!empty($userDetails)){
            $content = ucfirst($userDetails->Name);
          }
     ...
     public static function getUsernameForDomainLog($uid){
         Admincomponents::$dbObj = new Db();
         $userDetails = Admincomponents::$dbObj->selectRecord("User AS U LEFT JOIN ".Admincomponents::$dbObj->tablePrefix."domainrenewallog AS D ON U.nUId = D.nUId"," U.nUId,CONCAT_WS(' ', U.vFirstName, U.vLastName) as Name", " D.id = ".$uid);
         $data = Admincomponents::cmsUserNameDisplay($userDetails);
         return $data;
     }
...
//echopre($storeHost);
            if(!empty($storeHost)){
                $returnUrl = '<a href="http://'.$storeHost.'/admins/login/n/'.md5(SECRET_SALT.$storeHost).'" target="_blank">'.$storeHost.'</a>';
            }

        return $returnUrl;
     } // End Function

    public static function sendInvoiceMail($invoiceID, $appendMsg = NULL,$template='invoice'){        
        // Status - initially 0
        $status     = $discount = 0;
        ...
        if(count($invArr) > 0) {
            ...
            // Mail Message
            $mailMsgArr = Admincomponents::getListItem("Cms", array('cms_title','cms_desc'), array(array('field' => 'LOWER(cms_name)', 'value' => strtolower($template))));
            Logger::info($mailMsg);
            ...
            $emailData   = array("from"		=> COMPANY_EMAIL,
                    ...
                    "to"           => $userEmail);
            //echopre($emailData); 
            if($vSubscriptionType == 'PAID' || $vSubscriptionType == 'DUE'){ // Avoid sending invoice for free trial
                //$emailObj->email_senderNow($emailData);
                Mailer::sendSmtpMail($emailData);                
                $status = 1;
            }
            /* Send mail to store owner */
             ...
             /* Send a copy to admin by replacing the email content */
             ...
             $mailContent = preg_replace('/Dear \w+&nbsp;\w+,/','Dear Administrator,', $mailContent);
             $mailContent = str_replace("Thank you for the interest shown in ".COMPANY_NAME.".", "", $mailContent);
             $emailData["to"]       = ADMIN_EMAILS;
...
//echopre($lastBillArray);
    ...
    public static function getExpiredDomains() {
        ...
        $query = "SELECT PLU.nPLId FROM " . Admincomponents::$dbObj->tablePrefix . "Invoice I
                  ...
                  WHERE I.vSubscriptionType = 'FREE' AND I.upgraded = 0 AND IP.dDateNextBill <= CURDATE() AND PLU.nStatus != 0 ORDER BY nPLId DESC  LIMIT 5";//Edited by Anoop adde limit and order by for test purpose
    ...
    public static function logDomainRenewal($productLookUpID,$status, $logID = NULL,$dDateStop = NULL){       
        Admincomponents::$dbObj = new Db();
        $storeHost = $logStatus = $comments = "";
            if($productLookUpID){            
                $storeHost = Admincomponents::getStoreHost($productLookUpID);
                ...
                if(empty($dDateStop)){ //If end date is not specified
                    $sel        = "SELECT `dDateStop` FROM ".Admincomponents::$dbObj->tablePrefix."InvoiceDomain WHERE nPLId='".$productLookUpID."' ORDER BY dDateStop DESC LIMIT 0,1";
                    $dataArr    = Admincomponents::$dbObj->selectQuery($sel);
                    if(!empty($dataArr)){
                        $dDateStop = $dataArr[0]->dDateStop; 
                    }
                }
                ...
                if(!empty($domain_registrar) && !empty($userAccountInfoArr) && !empty($serverInfoArr)){
                    ...
                    switch($domain_registrar){
                        ...
                        case 'GODADDY':
                            ...
                            $userDetails            = User::$dbObj->selectRecord("User", "*", "nUId=".intval($userAccountInfoArr->nUId));
                            ...
                            if(trim($nShopperId) == ""){ 
                                $arrShopperDetails = array(
                                    ...
                                    "externalId"    => 0,
                                    "marketId"      => "en-US",
                                    "nameFirst"     => $userAccountInfoArr->vFirstName,
                                    "nameLast"      => $userAccountInfoArr->vLastName,
                                ...
                                //echo "Shopper Create API = <pre>"; print_r($arrJsonResponse); echo "</pre>";
                            ...
                            $arrDomRegnDetails["period"]    = $duration;
                            ...
                            $url            = $apiUrl."/v1/domains/".$domainName."/renew";
                            $ch             = curl_init();
                            curl_setopt($ch, CURLOPT_URL, $url);            
                            ...
                            $arrJsonResponse        = array();  
                            $arrJsonResponse        = json_decode($resp,1);
                            //echo "Domain Renewal API = <pre>"; print_r($arrJsonResponse); echo "</pre>"; //die();
    ...
    public function getProductLookupDetails($productLookUpID)
    {
        Admincomponents::$dbObj     = new Db();
        $listDataArr = array();
        if(!empty($productLookUpID)){ // User
        $resData = Admincomponents::$dbObj->execute("SELECT  PL.* FROM " . Admincomponents::$dbObj->tablePrefix . "ProductLookup PL  WHERE PL.nPLId=".$productLookUpID."");
    ...
    public static function getStoreWithUserDetailsFromProductLookupID($productLookUpID){
        ...
        $listDataArr = array();
        if(!empty($productLookUpID)){ // User
        $resData = Admincomponents::$dbObj->execute("SELECT PL.vSubDomain, PL.nSubDomainStatus, PL.vDomain, PL.nDomainStatus, PL.bluedogdetails,PL.nStatus, PL.vAccountDetails, PL.nTransactionSession, U.nUId, U.vFirstName, U.vLastName, U.vEmail, U.vInvoiceEmail, U.nStatus as userStatus, U.vPhoneNumber, U.vZipcode FROM " . Admincomponents::$dbObj->tablePrefix . "ProductLookup PL LEFT JOIN " . Admincomponents::$dbObj->tablePrefix . "User U ON PL.nUId = U.nUId WHERE PL.nPLId=".$productLookUpID." AND PL.nStatus=1 AND U.nStatus='1'");
