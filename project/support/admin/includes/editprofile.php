...
if ($_POST["postback"] == "" && $var_id != "") {
    ...
    $sql .= "nNotifyPvtMsg,nNotifyKB,vType,tSignature from sptbl_staffs where nStaffId = '" . mysql_real_escape_string($var_id) . "' AND vDelStatus='0' ";
    ...
    if (validateUpdation() == true) {
        if (!isUniqueEmail1($var_email, $var_id, "s")) {
            ...
            // for Host Manager section								
            if ($var_password != "") {
                $sql = "UPDATE autohoster_lookup SET vLookUpValue = '" . md5(addslashes($var_password)) . "' 
												   WHERE vLookUpName = 'admin_pass'";												   
                executeQuery($sql, $conn);
            }
            $sql = "UPDATE autohoster_lookup SET vLookUpValue = '" . addslashes($var_email) . "' 
										   WHERE vLookUpName = 'admin_mail'";
            executeQuery($sql, $conn);
            $sql = "UPDATE autohoster_lookup SET vLookUpValue = '" . addslashes($var_staffName) . "' 
										   WHERE vLookUpName = 'admin_name'";
            executeQuery($sql, $conn);
            $sql = "UPDATE autohoster_lookup SET vLookUpValue = '" . addslashes($var_signature) . "' 
										   WHERE vLookUpName = 'admin_signature'";
            executeQuery($sql, $conn);
            // for Host Manager section	ends here
...
<?php echo $var_message ?></td>
            ...
            <!-- <tr>
                ...
                <td width="55%" align="left">
                    <select name="cmbCssId" class="comm_input input_width1">
                    <?php echo($lst_css); ?>
                    </select>
                </td>
            </tr>
