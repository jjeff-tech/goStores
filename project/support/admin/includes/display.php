...
if ($_POST["postback"] == "D") {
    if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true and mysql_real_escape_string($_POST["id"]) !="1") {
        $sql="SELECT vCSSURL    FROM sptbl_css WHERE   nCSSId ='" . mysql_real_escape_string($_POST["id"]) . "'";
        ...
        $sql = "delete from  sptbl_css   where   nCSSId  ='" . mysql_real_escape_string($_POST["id"]) . "'";

        executeQuery($sql,$conn);
        //Insert the actionlog
        if(logActivity()) {
            $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','CSS','" . mysql_real_escape_string($_POST["id"]) . "',now())";
            executeQuery($sql,$conn);
        }
...
if($var_search != "") {
    if($var_cmbSearch == "title") {
        $qryopt .= " AND vCSSName like '%" . mysql_real_escape_string($var_search) . "%'";
    }elseif($var_cmbSearch == "pdate") {

        $qryopt .= " AND date_format(dDate,'%m/%d/%Y %H:%i:%s') like '" . mysql_real_escape_string($var_search) . "%'";
    }elseif($var_cmbSearch == "fname") {
        $qryopt .= " AND   vCSSURL  like '%" . mysql_real_escape_string($var_search) . "%'";
    }
}
...
<form name="frmDisplay" action="<?php echo($_SERVER["REQUEST_URI"]); ?>" method="post">
