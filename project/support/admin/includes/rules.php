...
//	if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true) {
    $sql = "delete from  sptbl_rules  where nRuleId='" . mysql_real_escape_string($_POST["id"]) . "'";
...
elseif ($_POST["postback"] == "DA") {
    ...
    if ($var_list != "") {
        ...
        if(logActivity()) {
            for($i=0;$i<count($_POST["chk"]);$i++) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Rules','" . mysql_real_escape_string($_POST["chk"][$i]) . "',now())";
                executeQuery($sql,$conn);
            }
        }
        $var_message = MESSAGE_RECORD_DELETED;
...
if($var_search != "") {
    if($var_cmbSearch == "name") {
        $qryopt .= " AND r.vRuleName like '" . mysql_real_escape_string($var_search) . "%'";
