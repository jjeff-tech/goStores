...
function datetimetomysql($vdate,$explodeType="-") {
    $vdate_ar=explode(" ",$vdate);
    $split_date=explode($explodeType,$vdate_ar[0]);
...
function isUniqueEmailUser($email,$var_id=0,$var_compid=0) {
    ...
    $sql = "Select * from dummy d
		...
		Left JOIN sptbl_staffs s on (d.num=1 AND  s.vDelStatus = '0' AND s.vMail='" . mysql_real_escape_string($email) . "')
...
function isUniqueEmail($email,$var_id=0,$var_type="") {
    ...
    if($var_id != 0) {
        switch($var_type) {
            case "u":
                $var_str_user = " AND u.nUserId != '" . mysql_real_escape_string($var_id) . "'";
                break;
            case "s":
                $var_str_staff = " AND s.nStaffId != '" . mysql_real_escape_string($var_id) . "'";
                break;
            case "d":
                $var_str_dept = " AND dt.nDeptId != '" . mysql_real_escape_string($var_id) . "'";
                break;
            case "c":
                $var_str_comp = " AND c.nCompId != '" . mysql_real_escape_string($var_id) . "'";
                break;
        }
    }
    $sql = "Select * from dummy d
		Left join sptbl_users u on (d.num=0 AND u.vEmail='" . mysql_real_escape_string($email) . "'{$var_str_user})
		Left JOIN sptbl_staffs s on (d.num=1 AND s.vDelStatus = '0' AND s.vMail='" . mysql_real_escape_string($email) . "'{$var_str_staff})
		Left join sptbl_depts dt on (d.num=2 AND dt.vDeptMail='" . mysql_real_escape_string($email) . "'{$var_str_dept})
		Left join sptbl_companies c on(d.num=3 AND c.vCompMail='" . mysql_real_escape_string($email) . "'{$var_str_comp})
		where d.num < 4  AND (u.nUserId IS NOT NULL OR s.nStaffId IS NOT NULL OR dt.nDeptId IS NOT NULL OR c.nCompId IS NOT NULL)";
    if(mysql_num_rows(executeSelect($sql,$conn)) > 0) {
        return false;
    }
...
function isUniqueEmail1($email,$var_id=0,$var_type="") {
    ...
    $sql = "Select * from dummy d
		Left join sptbl_users u on (d.num=0 AND u.vEmail='" . mysql_real_escape_string($email) . "'{$var_str_user})
		Left JOIN sptbl_staffs s on (d.num=1 AND s.vMail='" . mysql_real_escape_string($email) . "'{$var_str_staff})
		Left join sptbl_depts dt on (d.num=2 AND dt.vDeptMail='" . mysql_real_escape_string($email) . "'{$var_str_dept})
		Left join sptbl_companies c on(d.num=3 AND c.vCompMail='" . mysql_real_escape_string($email) . "'{$var_str_comp})
		where d.num < 4  AND (u.nUserId IS NOT NULL OR s.nStaffId IS NOT NULL OR dt.nDeptId IS NOT NULL OR c.nCompId IS NOT NULL)";
    if(mysql_num_rows(executeSelect($sql,$conn)) > 0) {
...
function sendMailUserTicketClose($mail_refno) {
    ...
    if(mysql_num_rows($result_user) > 0) {
        ...
        $var_body .= TEXT_CLOSED_BODY ." ". $mail_refno . TEXT_MAIL_BY . htmlentities($_SESSION['sess_stafffullname']) ."<br><br>";
    ...
    if($var_list != "") {
        ...
        if($resultCount !=1 ){
            ...
            if(mysql_num_rows($resultTicket) > 0) {
                $i =0;
                while($row = mysql_fetch_assoc($resultTicket)) { 
