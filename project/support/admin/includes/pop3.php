...
if ($_POST["postback"] == "D") {
    $sql = "delete from  sptbl_pop3settings  where nPOP3Id='" . mysql_real_escape_string($_POST["id"]) . "'";
    executeQuery($sql,$conn);
    //Insert the actionlog
    if(logActivity()) {
        $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','POP3','" . mysql_real_escape_string($_POST["id"]) . "',now())";
        executeQuery($sql,$conn);
    }
...
elseif ($_POST["postback"] == "DA") {
    ...
    for($i=0;$i<count($_POST["chk"]);$i++) {
        ...
        /*		        $qry="select * from sptbl_rules where nRuleId='".mysql_real_escape_string($_POST["chk"][$i])."'";
    ...
    if ($var_list != "") {
        ...
        if(logActivity()) {
            for($i=0;$i<count($_POST["chk"]);$i++) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','POP3','" . mysql_real_escape_string($_POST["chk"][$i]) . "',now())";
                executeQuery($sql,$conn);
            }
        }
        $var_message = MESSAGE_RECORD_DELETED;
...
if($var_search != "") {
    if($var_cmbSearch == "email") {
        $qryopt .= " AND p.vDeptEmail like '" . mysql_real_escape_string($var_search) . "%'";
    }elseif($var_cmbSearch == "server") {
        $qryopt .= " AND p.vServerName like '" . mysql_real_escape_string($var_search) . "%'";
    }
}
                ...
                <td><table width="100%"  border="0" cellpadding="0" cellspacing="0" >
                    ...
                    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            ...
                            <td class="pagecolor"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                                    <tr>
                                        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                                                <tr  class="listingbtnbar">
