...
if ($_POST["postback"] == "D") {
    if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true and mysql_real_escape_string($_POST["id"]) != "en" and  mysql_real_escape_string($_POST["id"]) !=$_SESSION["sess_defaultlang"]) {
        $sql = "delete from  sptbl_lang  where vLangCode='" . mysql_real_escape_string($_POST["id"]) . "'";
        executeQuery($sql,$conn);
        //Insert the actionlog
        if(logActivity()) {
            $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Language','" . mysql_real_escape_string($_POST["id"]) . "',now())";
            executeQuery($sql,$conn);
        }
...
if($var_search != "") {
    if($var_cmbSearch == "code") {
        $qryopt .= " Where vLangCode like '" . mysql_real_escape_string($var_search) . "%'";
    }elseif($var_cmbSearch == "desc") {
        $qryopt .= " Where vlangDesc like '" . mysql_real_escape_string($var_search) . "%'";
    }
}
...
<form name="frmLang" action="<?php echo($_SERVER["REQUEST_URI"]); ?>" method="post">
                                    ...
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                        ...
                                        <tr>
                                            <td align="left" class="whitebasic">
                                                ...
                                                <div class="left">
                                                    <input type="text" name="txtSearch" value="<?php echo(htmlentities($var_search)); ?>" class="inputstyle" onKeyPress="if(window.event.keyCode == '13'){ return false; }" style="width:140px; height:15px!important; margin-right:5px;"></div>
                                                <div class="left">
