...
if($_POST["del"] == "MS") {
    ...
    if(mysql_num_rows($result_spam) > 0) {
        while($row_spam = mysql_fetch_array($result_spam)) {
            ...
            $vuname = $row_spam['vUserName'];
            $var_machineip = $row_spam['vMachineIP'];
            $var_message_main = $row_spam['tQuestion'];
            $var_ticketid = $row_spam['nTicketId'];

            $sql = "insert into sptbl_spam_tickets(nSpamTicketId,vuseremail,nDeptId,vTitle,tQuestion,dPostDate,vMachineIP, nTicketID)
					values('','".mysql_real_escape_string($vuname)."','" . $val . "','".mysql_real_escape_string($row_spam['vTitle'])."','" . mysql_real_escape_string($var_message_main). "',now(),
					'" . mysql_real_escape_string($var_machineip) . "','".mysql_real_escape_string($var_ticketid)."')";
            executeQuery($sql,$conn);

            $sql = "Update sptbl_tickets set vDelStatus = 2 where nTicketId IN ($var_list)";
...
if(isset($_GET['pagenum']) != 'yes') {
    if($_GET['sorttype'] == 'DESC') {
        ...
        $var_sorttype = ($ticketListingOrder==0)?'DESC':'ASC';
        $var_filename = ($ticketListingOrder==0)?'s_desc.png':'s_asc.png';
    }
}else {
...
<div class="content_section">
    <form name="frmDetail" action="tickets.php?mt=<?php echo $_GET["mt"]; ?>&tp=<?php echo $_GET["tp"]; ?>&stylename=STYLETICKETS&styleminus=minus9&styleplus=plus9&" method="post">
        <div class="content_search_container">
            <div class="left rightmargin topmargin">
                <select name="cmbDepartment" class="comm_input input_width1" onChange="javascript:changeDepartment();">
                    ...
                    <?php
                    $sql2 = "Select d.nDeptId,d.vDeptCode,d.vDeptDesc as 'description', c.vCompName from
                             sptbl_depts d  inner join sptbl_companies c on d.nCompId = c.nCompId  ";
                    $lst_dept_opt = "";
        ...
        <?php
        if($_POST["del"] == "DM" or $_POST["up"] == "UP" or $_POST["labelup"] == "LABELUP" or $message !="" or $_POST["del"] == "MERGE") {
            echo("<div ".$flag_msg.">" . $message . "</div><br/>");
        }
        ?>
        <div>
            <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="list_tbl">
                ...
                <tr align="left">
                    ...
                    <th width="4%" align="left" >&nbsp;<?php echo "Preview"?></th>
                    <?php
                    $cnt = 0;
                                ...
                                case "vPriority":
                                    for($j=0;$j < count($fld_prio);$j++) {
                                        ...
                                        if ($fld_prio[$j][0] == $row[($fld_arr[$i][0])]) {
                                            echo ("<td width='12%' bgcolor=" . $fld_prio[$j][1] . " align='center'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . $fld_prio[$j][2] . "</a></td>");
                                        }
                                    }
                                    break;
                                case "dPostDate":
                                    echo "<td width='12%'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . date("m/d/Y H:i:s",strtotime($row[($fld_arr[$i][0])])) . "</a></td>";
                                    break;
                                case "nLockStatus":
                    ...
                    <td colspan="<?php echo $cnt+2; ?>" width="100%" class="subtbl">
                        ...
                        <div class="content_search_container">
                            ...
                            <div class="left rightmargin">
                                <select name="cmbAction" class="comm_input11b input_width1" >
                            ...
                            <div class="left">
                                <select name="cmbStatus" class="comm_input11b input_width1">
                            ...
                            <div class="left">
                                <select name="cmbLabel" class="comm_input11b input_width1">
