...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "SA") {
		...
		$sql = "Select distinct(vEmail) from sptbl_users where vDelStatus='0' AND vEmail != ''";
		$result = executeSelect($sql,$conn);
		if (mysql_num_rows($result) > 0) {
			$var_cnt = 0;
			while($row = mysql_fetch_array($result)) {
		...
		mysql_free_result($result);			
		$var_message = TEXT_EMAIL_CONFIRM;
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0" class="column1">
     ...
     <td>
		...
		?>	
         ...
         <input name="txtSubject" type="text" class="comm_input input_width2" id="txtSubject" size="72" maxlength="100" value="" style="font-size:11px;width:500px">
         </td>
                      </tr>
                      <tr><td colspan="3">&nbsp;</td></tr>
                      <tr>
                      ...
                      <td width="77%" align="left" >
                        <textarea name="txtBody" cols="71" rows="12" id="txtBody" class="textarea" style="font-size:11px;width:500px"></textarea></td>
                      </tr>
