...
*/
/*
	We have to check whether this user is valid, department is valid, priority is valid
*/
...
//				$var_refno=$var_compid."-".$deptid."-".$var_userid."-".$var_insert_id;
            ...
            if($vAttachmentfiles !="") {
                ...
                foreach($vAttacharr as $key=>$value) {
                    ...
                    $sql_insert_attach .= "('$var_ticketid','".mysql_real_escape_string($split_name_url[1])."','".mysql_real_escape_string($split_name_url[0])."'),";
                }
            ...
            if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nUserId,vAction,vArea,nRespId,dDate) Values('','$var_userid','" . mysql_real_escape_string(TEXT_ADDITION) . "','Ticket','" . mysql_real_escape_string($var_ticketid) . "',now())";
//                                           echo $sql."<br>";
                executeQuery($sql,$conn);
            }
            //$sql = "Insert into sptbl_actionlog(nALId,nUserId,vAction,vArea,nRespId,dDate) Values('','$var_userid','" . TEXT_ADDITION . "','Attachments','" . mysql_real_escape_string($var_insert_id) . "',now())";
            // executeQuery($sql,$conn);
            ...
            */
            $sqlWatcher  = "select DISTINCT vStaffname,vMail from sptbl_staffs s inner join sptbl_staffdept sd on s.nStaffId=sd.nStaffId ";
...
//				 $var_tolist = substr($var_tolist,1);
                ...
                $var_subject = '';
                if($var_tolist != "") {
                    $var_mail_body=$var_emailheader."<br>Hi $var_staff_name,<br>";
                    $var_mail_body .= "<br><br>";
                    ...
                    if($_SESSION["sess_smtpsettings"] == 1) {
                        ...
                        //echo $var_fromMail,$var_tolist,$var_smtpserver,$var_port,$var_subject,$var_body;
                        SMTPMail($var_fromMail,$var_tolist,$var_smtpserver,$var_port,$var_subject,$var_body);
                    }
                    else {//echo $var_tolist,$var_subject,$var_body,$Headers;
            ...
            /*
				  ...
				  * email is being sent to the user and all the staff of the department.
				  ...
				  $sql = "Select s.vLogin,s.vMail from sptbl_staffdept sd inner join sptbl_staffs s on
						sd.nStaffId = s.nStaffId where sd.nDeptId='" . $deptid . "' AND s.nNotifyArrival='1'";
					$result = executeSelect($sql,$conn);
...
<div class="content_section">
    ...
    <form name="frmDownloads" action="<?php echo($_SERVER["REQUEST_URI"]); ?>" method="post">

        <div class="content_section_title">
            <h3>
                <?php echo HEADING_TICKET_POSTED?>
            </h3>
        </div>


        <div class="content_section_data" align="center">
            <?php echo $var_message.' '.$var_message1; ?>
        </div>
...
if($var_post_flag==true && $var_continue_exec == true && $var_final_flag == true) {

    //Get email of user

    $useremail = getUserEmail($_SESSION["sess_userid"]);
    if (!in_array($_SESSION["sess_useremail"],$useremail)) {
    ...
    if(count($useremail) > 0) {
        ...
        foreach ($useremail as $key => $value) {
            ...
            /*				 $var_mail_body=$var_emailheader."<br>".TEXT_MAIL_START."&nbsp;".mysql_real_escape_string($var_username).",<br>".
            ...
            */
            ...
            if(isAutoReturnMailNeeded()) {
                $sql = "Select nDeptId,vDeptMail from sptbl_depts where nDeptId='" . mysql_real_escape_string($dept_id) . "'";
                $result = executeSelect($sql,$conn);
                if(mysql_num_rows($result) > 0) {
                    $row = mysql_fetch_array($result);
                    $var_body = $var_emailheader ."<br>".TEXT_MAIL_START.",<br>&nbsp;<br>";
 ...
 ************************************************************************************/
    ...
    /*$sql=" select s.nStaffId,s.vStaffname,s.vMail,s.vSMSMail,d.nDeptId from
	   sptbl_staffs s,sptbl_staffdept d where d.nDeptId=$dept_id and s.nStaffId=d.nStaffId 
	   and s.nNotifyArrival='1' and s.vDelStatus='0'";*/

    $sql=" select s.nStaffId,s.vStaffname,s.vMail,s.vSMSMail,d.nDeptId from
	   sptbl_staffs s,sptbl_staffdept d where d.nDeptId=$dept_id and s.nStaffId=d.nStaffId
	   and s.nNotifyArrival='1' and s.vDelStatus='0' and s.nWatcher !=1 ";
