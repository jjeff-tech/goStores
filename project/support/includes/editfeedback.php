...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "" && $var_id != "") {
		$sql = "SELECT f.nFBId,f.nTicketId,f.vFBTitle,f.tFBDesc
				FROM sptbl_feedback f INNER JOIN sptbl_tickets t ON f.nTicketId = t.nTicketId
		 		WHERE f.nFBId='" . mysql_real_escape_string($var_id) . "' AND t.nUserId = '".mysql_real_escape_string($_SESSION["sess_userid"])."' ";
		//echo $sql;
		...
		if (validateAddition() == true)	{
			$sql = "INSERT INTO sptbl_feedback(nFBId,nTicketId,
					...
					VALUES('','".mysql_real_escape_string($var_ticket_id)."',
	...
	elseif ($_POST["postback"] == "D"){
		if (validateDeletion() == true)	{
			...
			$sql = "Delete from sptbl_feedback where nFBId='" . mysql_real_escape_string($var_id) . "' ";
			executeQuery($sql,$conn);
			
			if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nUserId,vAction,vArea,nRespId,dDate) Values('','$var_userid','" . TEXT_DELETION . "','User Feedback','" . mysql_real_escape_string($var_id) . "',now())";			
			executeQuery($sql,$conn);
			}
	...
	elseif ($_POST["postback"] == "U") {
		if (validateUpdation() == true)	{
			$sql = "UPDATE sptbl_feedback SET 
					vFBTitle='" . mysql_real_escape_string($var_title) . "',
					tFBDesc='" . mysql_real_escape_string($var_desc) . "'
					 WHERE nFBId='" . mysql_real_escape_string($var_id) . "'";
			executeQuery($sql,$conn);
			//echo $sql;
			if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nUserId,vAction,vArea,nRespId,dDate) Values('','$var_userid','" . TEXT_UPDATION . "','User Feedback','" . mysql_real_escape_string($var_id) . "',now())";			
			executeQuery($sql,$conn);
			}
