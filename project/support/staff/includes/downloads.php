...
if ($_POST["postback"] == "D") {
    if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true) {
        $sql="SELECT vURL    FROM sptbl_downloads WHERE   nDLId ='" . mysql_real_escape_string($_POST["id"]) . "'";
        $rs_oldurl = executeSelect($sql,$conn);
        $rowoldurl=mysql_fetch_array($rs_oldurl);
        $oldurl=$rowoldurl['vURL'];
        unlink("../".$oldurl);
        executeQuery($sql,$conn);
        $sql = "delete from  sptbl_downloads   where nDLId  ='" . mysql_real_escape_string($_POST["id"]) . "'";
        executeQuery($sql,$conn);
...
if($var_search != "") {
    if($var_cmbSearch == "title") {
        $qryopt .= " AND vDescription like '" . mysql_real_escape_string($var_search) . "%'";
        ...
        $qryopt .= " AND  vURL  like 'downloads/" . mysql_real_escape_string($var_search) . "%'";
    }
}
    ...
    <div class="content_section">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="whitebasic">
            ...
            <div style="background-color:#ffffff; " class="content_search_container">
                ...
                &nbsp;
                <div class="left" style="margin-top:0px !important; padding-top:0px !important; height:18px; ">
                    <input type="text" name="txtSearch" value="<?php echo(htmlentities($var_search)); ?>" class="comm_input_txtbx" onKeyPress="if(window.event.keyCode == '13'){ return false; }"  style="width:140px; height:19px!important; margin-right:5px;">
                   </div>
                <div class="left">
