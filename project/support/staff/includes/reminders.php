...
if ($_POST["postback"] == "D") {
    if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true) {
        ...
        $sql = "delete from sptbl_reminders where nRemId = '" . mysql_real_escape_string($_POST["id"]) . "'";
        executeSelect($sql,$conn);

        //Insert the actionlog
        if(logActivity()) {
            $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Reminders','" . mysql_real_escape_string($_POST["id"]) . "',now())";
...
elseif ($_POST["postback"] == "DA") {
    ...
    if (validateDeletion($var_list) == true) {
        ...
        if(logActivity()) {
            for($i=0;$i<count($_POST["chk"]);$i++) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Reminders','" . mysql_real_escape_string($_POST["chk"][$i]) . "',now())";
                executeQuery($sql,$conn);
            }
        }
        $var_message = MESSAGE_RECORD_DELETED;
...
$qryopt='';
if($_POST["txtSearch"] != "") {
    $var_search = mysql_real_escape_string(trim($_POST["txtSearch"]));
}else if($_GET["txtSearch"] != "") {
    $var_search = mysql_real_escape_string(trim($_GET["txtSearch"]));
}

if($_POST["cmbSearch"] != "") {
    $var_cmbSearch = mysql_real_escape_string(trim($_POST["cmbSearch"]));
}else if($_GET["cmbSearch"] != "") {
    $var_cmbSearch = mysql_real_escape_string(trim($_GET["cmbSearch"]));
}
