...
}else if ($_POST["postback"] == "S") { //echo '<pre>'; print_r($_POST); echo '</pre>';
    ...
    */
    ...
    if($validsave =="1") {
        ...
        if($var_addtokb=="atokb" and $var_category >0) {
            ...
            if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate,logStatus) Values('','" . $_SESSION["sess_staffid"] . "','" . TEXT_ADDITION . "','Knowledgebase','" . mysql_real_escape_string($var_insert_id) . "',now(),'N')";
                executeQuery($sql,$conn);
...
//							// it is for smtp mail sending
        ...
        if(logActivity()) {
            $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate,logStatus) Values('','" . $_SESSION["sess_staffid"] . "','" . TEXT_UPDATION . "','Ticket','" . mysql_real_escape_string($var_tid) . "',now(),'N')";
            executeQuery($sql,$conn);
            $actionLogId = mysql_insert_id();
        }
...
//echo "<pre>";print_r($_SESSION["sess_backurl_reply_success"]);echo "</pre>";exit;
...
exit;
    ...
    */		 if($validsave =="1") {
        ...
        if($var_hold=='' || $var_hold==0) {
            ...
            $sql_reply_up = "update sptbl_replies set tReply='".mysql_real_escape_string($var_replymatter)."',vReplyTime='".mysql_real_escape_string($var_timespent)."',nReplyStatus='".mysql_real_escape_string($var_status)."' where nReplyId='".$var_reply_id."'";
            executeQuery($sql_reply_up,$conn);
            $var_insert_id = $var_reply_id;
        }
        //Insert the actionlog
    ...
    function getTemplate(){
        ...
        $.ajax({
            ...
            success		:function(response){
               // alert(response);
               // var oEditor =CKEDITOR.instances['txtRpMatter'];
                // oEditor.InsertHtml(" ");
                if(response!='')
                {
                    //oEditor.SetHTML(response);
                    CKEDITOR.instances.txtRpMatter.setData( response);
                }
                else
                {
                  CKEDITOR.instances.txtRpMatter.setData('');
                }
    ...
    <div class="content_section">
        ...
        <div class="content_section_data">
            ...
            <table width="100%"  border="0" cellspacing="0" cellpadding="0" class="whitebasic">
                <tr>
                    <td width="100%" align="right" colspan=3 class="whitebasic" style="padding-right:10px;"><?php echo TEXT_FIELDS_MANDATORY ?></td>
                </tr>
                <tr>
                              ...
                              <!--  <tr>
                                ...
                                <tr>
                                    ...
                                    <td width="60%" align="left" class="listingmaintext">
                                        ...
                                        <input name="btnSubmit" type="button" class="comm_btn" style="float:none; " value="<?php echo BUTTON_TEXT_ATTACH?>" onClick="javascript:attach();">
                                            <?php
                            ...
                            <tr >
                                ...
                                <td >
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="width:99%;">
                                        <tr>
                                            <td width="100%">
                                                <div style="overflow:auto;border:1px solid #666;padding:6px; width:100%">
