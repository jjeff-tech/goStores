...
/* Modified on 28-07-09
$sql = "Select t.nTicketId,t.nUserId,t.nDeptId,d.vDeptDesc,t.vRefNo,t.vUserName,t.vTitle,t.tQuestion,t.vPriority,
		t.dPostDate,t.vStatus,s.vStaffname as 'vOwner',t.nLockStatus,t.dLastAttempted,rp.nStaffId,rp.nUserId as rpuserid from
		...
		sptbl_staffs s on t.nOwner = s.nStaffId  left join sptbl_replies rp on (rp.dDate=t.dLastAttempted and rp.nTicketId=t.nTicketId) WHERE  ";
*/
$sql = "Select t.nTicketId,t.nUserId,t.nDeptId,d.vDeptDesc,t.vRefNo,t.vUserName,t.vTitle,t.tQuestion,t.vPriority,
		t.dPostDate,t.vStatus,s.vStaffname as 'vOwner',t.nLockStatus,t.dLastAttempted,rp.nStaffId,rp.nUserId as rpuserid, t.vViewers from
		sptbl_tickets t inner join sptbl_depts d on t.nDeptId = d.nDeptId 
		left outer join sptbl_labels l on t.nLabelId = l.nLabelId  left outer join
...
$qryopt="";
$flag_qry = false;
$txtinteligentsearch = $_REQUEST['txtinteligentsearch'];
$txtinteligentsearch = trim($txtinteligentsearch);

//if($txtinteligentsearch !="" && $_POST['btninteligentsearch']) {
if($txtinteligentsearch !="") { 
    $var_logical = " OR ";
    $flag_qry = true;
    //$var_operator = "=";
    // $qryopt .=  $var_logical . "  d.nCompId" . $var_operator . "'" . mysql_real_escape_string($txtinteligentsearch) . "' ";
    $var_operator = " LIKE ";

    //  $qryopt .=  $var_logical . "  d.vDeptDesc" . $var_operator . "'" . mysql_real_escape_string($txtinteligentsearch) . "' ";
    $qryopt .=  "  d.vDeptDesc" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $var_operator = " LIKE ";
    $qryopt .= $var_logical . "  t.vStatus" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $qryopt .= $var_logical . "  s.vStaffname" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $qryopt .= $var_logical . "  t.vUserName" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $qryopt .=  $var_logical . "  t.vRefNo" . $var_operator ."'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $qryopt .= $var_logical . " t.vTitle" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    $qryopt .= $var_logical . " u.vEmail" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
    //$var_operator = "=";
    $qryopt .=  $var_logical . "  l.vLabelname" . $var_operator . "'" . mysql_real_escape_string($txtinteligentsearch) . "' ";
    $qryopt .= $var_logical . " t.tQuestion" . $var_operator . "'%" . mysql_real_escape_string($txtinteligentsearch) . "%' ";
     //echo $qryopt;exit;
}
else { 
...
//******Advanced search ****************
    ...
    if ($var_company != "") {
        ...
        $qryopt .=  $var_logical . "  d.nCompId" . $var_operator . "'" . mysql_real_escape_string($var_company) . "' ";
    }
    if ($var_department != "") {
        ...
        $qryopt .=  $var_logical . "  d.nDeptId" . $var_operator . "'" . mysql_real_escape_string($var_department) . "' ";
    }
    if ($var_status != "") {
        ...
        $qryopt .= $var_logical . "  t.vStatus" . $var_operator . "'%" . mysql_real_escape_string($var_status) . "%' ";
    }
    if ($var_owner != "") {
        ...
        $qryopt .= $var_logical . "  s.vStaffname" . $var_operator . "'%" . mysql_real_escape_string($var_owner) . "%' ";
    }
    if ($var_user != "") {
        ...
        $qryopt .= $var_logical . "  t.vUserName" . $var_operator . "'%" . mysql_real_escape_string($var_user) . "%' ";
    }
    if ($var_ticketno != "") {
        ...
        $qryopt .=  $var_logical . "  t.vRefNo" . $var_operator ."'%" . mysql_real_escape_string($var_ticketno) . "%' ";
    }
    if ($var_title != "") {
        ...
        $qryopt .= $var_logical . " t.vTitle" . $var_operator . "'%" . mysql_real_escape_string($var_title) . "%' ";
    }
    if ($var_email != "") {
        ...
        $qryopt .= $var_logical . " u.vEmail" . $var_operator . "'%" . mysql_real_escape_string($var_email) . "%' ";
    }
    /*Added on 280709 starts*/
    ...
    if ($var_label != "") {
        ...
        $qryopt .=  $var_logical . "  l.nLabelId" . $var_operator . "'" . mysql_real_escape_string($var_label) . "' ";
    }
    /*Added on 280709 ends*/
    ...
    if ($var_question != "") {
        ...
        $qryopt .= $var_logical . " t.tQuestion" . $var_operator . "'%" . mysql_real_escape_string($var_question) . "%' ";
    }
    if ($var_from != "") {
        $var_logical = ($flag_qry == true)?"AND":"";
        $flag_qry = true;
        ...
        $var_time1 = $arr_year[0] ."-" . $arr_alert[0] . "-" . $arr_alert[1];// . " " . $arr_tm[0] . ":" . $arr_tm[1];exit;


        //$qryopt .=  $var_logical . "  t.dPostDate >='" . mysql_real_escape_string($var_time1) . "' ";
        $qryopt .=  $var_logical . "  DATE_FORMAT(t.dPostDate,'%Y-%m-%d') >='" . mysql_real_escape_string($var_time1) . "' ";
    ...
    if ($var_to != "") {
        ...
        //$qryopt .=  $var_logical . " t.dPostDate <= '" . mysql_real_escape_string($var_time2) . "' ";
        $qryopt .=  $var_logical . "  DATE_FORMAT(t.dPostDate,'%Y-%m-%d') <= '" . mysql_real_escape_string($var_time2) . "' ";
    }


//********Advanced search ends
}//else ends (chk advance search)

$qryopt = (strlen($qryopt) > 0)?(" ( " . $qryopt . " ) AND t.vDelStatus='0' AND t.nDeptId IN($lst_dept) "):" t.vDelStatus='0' AND t.nDeptId IN($lst_dept)";
...
<div class="content_section">
    <form name="frmDetail" action="<?php echo($_SERVER["REQUEST_URI"]); ?>" method="post">
        ...
        <div class="content_section_title">
            <h3><?php echo HEADING_ADVANCED_RESULT ?></h3>
        </div>
        <br>
        ...
        <table width="100%"  border="0" cellpadding="2" cellspacing="1" class="list_tbl"  >
            ...
            if($_POST["del"] == "DM" or $_POST["del"] == "UP" or $_POST["labelup"] == "LABELUP" or $message !="") {
                echo("<tr><td align=\"center\"  class=\"errormessage\" colspan=11><div ".$flag_msg.">" . $message . "</div></td></tr>");
            }
            ?>
            <tr align="left" >
                <th width="3%" align="center"><input name="checkall" id="checkall" type=checkbox class=checkbox onClick="checkallfn(<?php echo $var_maxposts;?>)">
                </th>
                ...
                while($cnt < count($fld_arr)) {
                    ...
                    ?>
                <th style="text-decoration:none;"><?php echo "<b><a href='?val=".$fld_arr[$cnt][0]."&txtinteligentsearch=$txtinteligentsearch&sorttype=".$var_sorttype."&tp=".$var_type."&cmbDepartment=".$var_deptid."&frm=$var_from&to=$var_to&numBegin=$var_numBegin&start=$var_start&begin=$var_begin&num=$var_num&mt=y&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus'>".constant($fld_arr[$cnt][1])."</a></b>&nbsp;&nbsp;".$img_path; ?></th>
                    <?php
                    $cnt++;
                }
                //$cnt++;
                $cnt += 2;
                ?>
                <th width="15%"  style="text-decoration:none;"><?php echo "<b>".TEXT_DUE."</b>";?></th>
            </tr>
            <?php
...
/////
            ...
            <tr align="left"  <?php echo $viewedClass;?>>
                <td align="center"><input type="checkbox" name="chkDeleteTickets[]" id="chkDeleteTickets<?php echo($cntr);?>" value="<?php echo($row["nTicketId"]); ?>" class="checkbox">
                </td>
                <td align="center">
                    ...
                    for($i=0;$i < count($fld_arr);$i++) {
                        switch($fld_arr[$i][0]) {
                                ...
                                for($j=0;$j < count($fld_prio);$j++) {
                                    ...
                                    if ($fld_prio[$j][0] == $row[($fld_arr[$i][0])]) {
                                        echo ("<td align=\"center\" width=10% class=\"subtbl\" style='background-color:$ticketColor;'><table width=\"100%\" cellpadding=\"0\" border=\"0\" class='innertable1' ><tr><td class='user_priority_icon'  align='left'><img src='$filePath'></td>
                                                                                                    <td bgcolor=" . $fld_prio[$j][1] . " align='center'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\">" . $fld_prio[$j][2] . "</a></td></tr></table></td>");
                                    }
                                }
                                break;
                            case "dPostDate":
                            ...
                            //echo "<td width='1%'  style=\"word-break:break-all;\" align=center><span  id=\"link".$count."\"  onMouseOver=\"displayAd($count," . $row["nTicketId"] . ");\" onMouseOut=\"hideAd();\" ><a href=\"viewticket.php?mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'><img src='./../images/ticketdetails.gif' border=0></span></a></td>";
                                ...
                                break;
                            case "vTitle":
                                echo "<td width='48%' style=\"word-break:break-all;\"><span style=\"width:100%;height:100%;\" id=\"link".$count."\" ><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" .  htmlentities($row[($fld_arr[$i][0])]) ."<br>".$arrayDeptName[$row["nDeptId"]]. "</a></span></td>";
                                break;
                            default:
                                echo "<td width='10%'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" .  htmlentities(ucwords($row[($fld_arr[$i][0])])) . "</a></td>";
                                break;
                        }
                        ?>
                        <?php
...
<script>
    ...
    var owner = '<?php echo(mysql_real_escape_string($var_owner)); ?>';
    var user = '<?php echo(mysql_real_escape_string($var_user)); ?>';
    var ticketno = '<?php echo(mysql_real_escape_string($var_ticketno)); ?>';
    var title = '<?php echo(mysql_real_escape_string($var_title)); ?>';
    var email = '<?php echo(mysql_real_escape_string($var_email)); ?>';
    var label = '<?php echo(mysql_real_escape_string($var_label)); ?>';
    var qst = '<?php echo(mysql_real_escape_string($var_question)); ?>';
