...
if ($_POST["mt"] == "u") {
    ...
    if (mysql_num_rows($rs) > 0) {
        ...
        if($frm_lock == "1" && $frm_ownerid != $var_staffid) {
            $update_flag = false;
            $var_message = MESSAGE_LOCK_OWNER_TICKET_UPDATE_ERROR;
            $flag_msg = "class='msg_error'";
        }
        else {
            $update_flag = true;
            ...
            if ($row["nOwner"] != $frm_ownerid || $row["nDeptId"] != $frm_deptid) {
                $sql = "Select s.vLogin,s.vStaffname,s.nNotifyAssign,s.vMail,s.vSMSMail,sd.nDeptId from sptbl_staffdept sd inner join sptbl_staffs s on
                                                        sd.nStaffId = s.nStaffId Where sd.nStaffId='" . mysql_real_escape_string($frm_ownerid) . "'
                                                         AND sd.nDeptId='" . mysql_real_escape_string($frm_deptid) . "' AND s.vDelStatus='0' AND s.vType != 'A' ";
...
<script type='text/javascript'>
    ...
    mysql_free_result($result);
    $sql="select nDeptid,vTitle,nUserId,tQuestion,vRefNo from sptbl_tickets where nTicketId='".mysql_real_escape_string($var_ticketid)."'";
//echo $sql;
    $result = executeSelect($sql,$conn);
    if (mysql_num_rows($result) > 0) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $sql = "Select t.nTicketId,t.vTitle,t.tQuestion,t.dPostDate,t.vMachineIP,
			...
			where d.num < 2  AND (t.nTicketId IS NOT NULL OR r.nReplyId IS NOT NULL and r.eReplySentstatus <> 'N') order by r.dDate ";
        ...
        while($row = mysql_fetch_array($rs)) {
            if($row["nTicketId"] != "") {  //Ticket section
                ...
                if ($var_staffid == $row['nStaffId'] && trim($row['tPvtMessage']) != '') {
                    ...
                    $private = nl2br(htmlentities($row['tPvtMessage']));
            ...
            <td>
                ...
                <div class="list_tbl_history">
                    ...
                    <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="list_tbl">
                        <?php
//***********************HISTORY SECTION**************************
                        $sql = "Select t.nTicketId,t.nUserId,t.vTitle,t.vRefNo,t.vStatus,t.dPostDate,t.nDeptId,rp.nStaffId,rp.nUserId as rpuserid
                                from sptbl_tickets t
                                left join sptbl_replies rp
                                on (rp.dDate=t.dLastAttempted and rp.nTicketId=t.nTicketId)
                                WHERE t.nUserId='" . mysql_real_escape_string($var_userid) . "'
                                AND t.vDelStatus='0' ";
                        ...
                        <tr>
                            ...
                            <td align="left" valign="top"><?php echo date("m-d-Y  H:i:s",strtotime($row["dPostDate"])); ?> </td>
                        </tr>
                            <?php
                            $count++;
                        }
    ...
    <div style="overflow:auto;">
		...
		$var_close_url = "<a href=\"viewticket.php?mt=y&cl=y&tk=" . $var_ticketid . "&stylename=" . $var_stylename . "&styleminus=" . $var_styleminus . "&styleplus=" . $var_styleplus . "&\" class=\"linktext\">" . TEXT_CLOSE_TICKET . "</a>";
            ...
            $var_reply_idlist = "";
            $i=0;
            while($row = mysql_fetch_assoc($rs)) { //echo '<pre>'; print_r($row); echo '</pre>';
                if($row["nReplyId"] != "") {
                    $var_reply_idlist .= "," . $row["nReplyId"];
                }
                /*
                            ...
                            <div class="content_section_data" >
                                    ...
                                    <div class="left ticket_user_info">
                                        <table cellpadding="0" cellspacing="0" border="0" class="comm_tbl2" width="100%">
                                            <tr align="left">
                                                ...
                                                <td>
                                                <?php echo $row["eReplySentstatus"];?>
                                                <?php echo " :<a href=\"javascript:userdetails('$var_userdetailid')\" > <b>" . htmlentities(stripslashes($var_username))."</a></b>"; ?> </td>
                                            </tr>
                                            <tr>
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="comm_tbl2">
                                    <tr>
                                        ...
                                        <td align="left" valign="top">:&nbsp;<b><?php echo htmlentities(stripslashes($row["vTitle"])); ?></b></td>
                                    </tr>
                                    <tr>
                    ...
                    <div class="content_section_data">
                        ...
                        <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="comm_tbl2">
                            <tr align="left">
                                <td colspan="4" width="10%" style="word-break:break-all;">
                                    <?php
                                    // echo html_entity_decode(htmlspecialchars(stripslashes(nl2br($row["tReply"]))));
                            ...
                            */
                            ...
                            ?>
                                ...
                                <tr>
                                    <td colspan=4 align="left" style="word-break:break-all;" class="listing"><?php echo nl2br(htmlentities(stripslashes($row["tPvtMessage"]))); ?></td>
                                </tr>
                                <?php
...
//  echo $sqlprevious
            ...
            <input type="button" class="comm_btn_greyad" name="btPrevious" id="btPrevious" value="<?php echo(TEXT_MAIN_PREVIOUS);?>" onClick="javascript:clickPrevious();"></a>
                    <?php	}
            }
...
//		  	  echo $sqlnext;
            ...
            &nbsp;&nbsp;&nbsp;
            <input type="button" class="comm_btn_greyad" name="btDelete" id="btDelete" value="<?php echo(TEXT_MAIN_DELETE); ?>" onClick="javascript:deleteTickets('<?php echo($var_ticketid); ?>');">
                <?php
            }
            ?>
            ...
            <input type="button" class="comm_btn" name="btNextReply" id="btNextReply" value="<?php echo(TEXT_REPLY_NEXT); ?>" onClick="javascript:clickReplyNext();">
                <?php } ?>
            &nbsp;&nbsp;&nbsp;
            <input type="button" class="comm_btn_greyad" name="btBack" id="btBack" value="<?php echo(TEXT_MAIN_BACK); ?>" onClick="javascript:clickBack();">
            &nbsp;&nbsp;&nbsp;
            <?php


            if(mysql_num_rows($rsnext)>0) {?>
            <input type="button" class="comm_btn_greyad" name="btNext" id="btNext" value="<?php echo(TEXT_MAIN_NEXT);?>" onClick="javascript:clickNext();"></a>
                <?php } ?>
...
// added on 1-11-06 by roshith
    ...
    $backurl='viewticket.php?limitval=0&mt=y&tk='.$var_tid.'&us='.$var_userid.'&stylename=STYLETICKETS&styleminus=minus1&styleplus=plus1';
