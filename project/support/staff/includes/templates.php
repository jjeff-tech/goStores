...
if ($_POST["postback"] == "D") {
    if (validateDeletion(mysql_real_escape_string($_POST["id"])) == true) {
        $sql = "delete from  sptbl_templates  where nTemplateId  ='" . mysql_real_escape_string($_POST["id"]) . "'";
        //echo "sql==$sql";
        executeQuery($sql,$conn);

        //Insert the actionlog
        if(logActivity()) {
            $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Templates','" . mysql_real_escape_string($_POST["id"]) . "',now())";
            executeQuery($sql,$conn);
        }
...
elseif ($_POST["postback"] == "DA") {
    ...
    if (validateDeletion($var_list) == true) {
        $sql = "delete from  sptbl_templates  where nTemplateId IN(" . $var_list . ")";
        ...
        if(logActivity()) {
            for($i=0;$i<count($_POST["chk"]);$i++) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Templates','" . mysql_real_escape_string($_POST["chk"][$i]) . "',now())";
                executeQuery($sql,$conn);
            }
        }
        $var_message = MESSAGE_RECORD_DELETED;
...
$sql .= "LEFT OUTER JOIN sptbl_staffs sf ON st.nStaffId = sf.nStaffId where  st.nStaffId=$var_staffid and st.vStatus='1' ";
...
$sql .= $qryopt . " Order By st.dDate desc,st.vTemplateTitle  Asc ";
...
<form name="frmTemplate" action="<?php echo($_SERVER["REQUEST_URI"]); ?>" method="post">
    <div class="content_section">
        <div class="content_section_title">
            <h3><?php echo HEADING_TEMPLATE_DETAILS ?></h3>
        </div>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="whitebasic">
            <tr><td width="100%">
                    ...
                    <div style="background-color:#ffffff; " class="content_search_container">
                        ...
                        &nbsp;
                        <div class="left">
                            <input type="text" name="txtSearch" value="<?php echo(htmlentities($var_search)); ?>" class="comm_input input_width3" onKeyPress="if(window.event.keyCode == '13'){ return false; }" style="width:140px; height:21px!important; margin-right:5px;">
                            &nbsp;&nbsp;</div>
                        <div class="left">
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="list_tbl" >
            <tr align="left"  class="listing">
                <th width="5%">&nbsp;</th>
                <th width="37%" ><?php echo "<b>".TEXT_TITLE."</b>"; ?></th>
                <th width="20%"><?php echo "<b>".TEXT_USER."</b>"; ?></th>
                <th width="27%"><?php echo "<b>".TEXT_DATE."</b>"; ?></th>
...
//echo("$totalrows,2,2,\"&ddlSearchType=$var_cmbSearch&txtSearch=" . urlencode($var_search) . "&id=$var_batchid&\",$var_numBegin,$var_start,$var_begin,$var_num");
            $navigate = pageBrowser($totalrows,10,10,"&mt=y&cmbSearch=$var_cmbSearch&txtSearch=" . urlencode($var_search) . "&stylename=STYLETEMPLATES&styleminus=minus5&styleplus=plus5&",$var_numBegin,$var_start,$var_begin,$var_num);
...
//$rs = mysql_query($sql,$conn);
            ...
            <tr align="left"  class="whitebasic">
                <td align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["nTemplateId"]); ?>" class="checkbox">
                </td>
                <td><?PHP echo htmlentities(trim_the_string($row["vTemplateTitle"])); ?>
                </td>

                <td><?php echo($row["vLogin"] == "")?"Administrator":$row['vLogin']; ?></td>
                <td><?php echo datetimefrommysql($row["dDate"]); ?></td>
                <td width="6%"><a href="edittemplate.php?id=<?php echo $row["nTemplateId"]; ?>&stylename=STYLETEMPLATES&styleminus=minus5&styleplus=plus5&"><img src="././../images/edit.gif" width="13" height="13" border="0" title="<?php echo TEXT_TITLE_EDIT ?>"></a></td>
                <td width="5%"><a href="javascript:deleted('<?php echo $row["nTemplateId"]; ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_TITLE_DELETE ?>"></a></td>
            </tr>
                <?php
                $cnt++;
            }
