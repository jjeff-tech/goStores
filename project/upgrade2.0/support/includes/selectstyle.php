...
if($_POST["postback"] == "Save Changes"){
        ...
        if($error){
                ...
                $sql1 .= " SET nCSSId = '".addslashes($newid)."' WHERE nUserId = '".$_SESSION["sess_userid"]."' ";
              ...
              $sql = "Select vCSSURL from sptbl_css where nCSSId='".addslashes($newid)."'";
...
$sql .=" where u.nUserId='".addslashes($_SESSION["sess_userid"])."'";
$result = executeSelect($sql,$conn);
if (mysql_num_rows($result) > 0) {
        $row = mysql_fetch_array($result);
