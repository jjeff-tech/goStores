...
if($_POST["postback"] == "Save Changes"){
	...
	if($error){
		...
		if($auth_Status =='1'){
			...
			$delStatus = 0;
			$infomessage = MESSAGE_REGISTRATION_SUCCESFULL;
		}
	
		$sql1  = " INSERT INTO sptbl_users(`nUserId`, `nCompId`,`vUserName`,`vEmail`,`vLogin`,`vPassword`,`dDate`, `vBanned`, `vDelStatus`) ";
		$sql1 .= " VALUES('','".addslashes($company)."', '".addslashes($name)."','".addslashes($email)."','".addslashes($loginname)."','".addslashes(md5($password))."',now(),'0','".addslashes($delStatus)."')";
...
<div class="content_section_title"><h3><?php echo TEXT_REGISTER?></h3></div>
												  <!-- ##########################################- -->
                            ...
                            <tr>
                              <td><table width="100%"  border="0" cellpadding="0" cellspacing="0" class="maintext">
                                  <tr align="center" class="pagecolor">
                                    <td width="36%">&nbsp;</td>
                                    <td width="1%" align="left"><input name="btnSubmit" type="button" class="comm_btn" value="<?php echo BUTTON_TEXT_SUBMIT?>" onClick="javascript:validateProfileForm();"></td>
                                    <td width="15%"><input name="btnCancel" type="button" class="comm_btn" value="<?php echo BUTTON_TEXT_CANCEL?>" onclick="window.location.href='index.php'"></td>
                                    <td width="34%">&nbsp;</td><td>
