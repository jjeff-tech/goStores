        ...
        $email = $_POST["txtEmail"];
		 if(!isValidEmail($email)){
		  ...
		  $sql = "Select * from sptbl_useremail u ";
                        $sql .=" where u.nUserId !='".addslashes($_SESSION["sess_userid"])."' AND vEmail = '". mysql_real_escape_string($email)."'";
...
if($error){
		$errormessage = MESSAGE_ERRORS_FOUND . "<br>" .$errormessage;
                ...
                $id= $_POST['id'];
               $sql1  = " UPDATE  sptbl_useremail SET vEmail = '". mysql_real_escape_string($email)."' where nUseremailId ='".addslashes($id)."' ";
		$result1 = executeQuery($sql1,$conn);
		$message = true;
    ...
    if(isNotNull($_GET["id"])){
        $id= $_GET['id'];
        $sql1  = " DELETE FROM  sptbl_useremail  where nUseremailId ='".addslashes($id)."' ";
        $result1 = executeQuery($sql1,$conn);
        $message = true;
    ...
    if(isNotNull($_GET["id"])){
         $id= $_GET['id'];
        $sql = "Select * from sptbl_useremail u ";
   $sql .=" where u.nUserId ='".addslashes($_SESSION["sess_userid"])."' AND nUseremailId ='".addslashes($id)."'";
...
<div class="content_section_title"><h3><?php echo HEADER_EMAL?></h3></div>
...
<div class="content_section_data">
    ...
    <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="list_tbl" style="border:1px solid #cfcfcf; margin-bottom:10px; ">
                            <tr>
                                <th align="left" width="90%"><?php echo TEXT_EMAIL;?></th>
                                <th align="center" width="10%"><?php echo TEXT_ACTION;?></th>
                            </tr>
                            <?php
                           
                            $sql = "Select * from sptbl_useremail u ";
                            $sql .=" where u.nUserId ='".addslashes($_SESSION["sess_userid"])."' AND u.vStatus = 'Y'";
                            $result = executeSelect($sql,$conn);
                            if (mysql_num_rows($result) > 0) {
