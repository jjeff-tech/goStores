...
if ($_POST["postback"] == "D") {
	if (validateDeletion(addslashes($_POST["id"])) == true) {
	    ...
	    $sql = "delete from  sptbl_labels  where nLabelId  ='" . addslashes($_POST["id"]) . "'";
	    executeQuery($sql,$conn);
		
	   //Insert the actionlog
	   if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Labels','" . addslashes($_POST["id"]) . "',now())";					
			executeQuery($sql,$conn);
		}
...
elseif ($_POST["postback"] == "DA") {
	...
	if (validateDeletion($var_list) == true) {
		$sqlUpdate = "UPDATE sptbl_tickets SET nLabelId=0 WHERE nLabelId IN(" . $var_list . ")";
		executeQuery($sqlUpdate, $conn);
		...
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Labels','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		$var_message = MESSAGE_RECORD_DELETED;
...
$sql .= "LEFT OUTER JOIN sptbl_staffs sf ON sl.nStaffId = sf.nStaffId where  sl.nStaffId=$var_staffid ";
//$sql = "Select * from sptbl_labels where 1=1  ";
...
if($var_search != ""){
	if($var_cmbSearch == "title"){
	        $qryopt .= " AND sl.vLabelname like '" . addslashes($var_search) . "%'";
	}
}
...
<form name="frmLabel" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post">
                       ...
                       <table width="100%" cellpadding="0" cellspacing="0" border="0" class="whitebasic">
                                          ...
                                          <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="list_tbl">
                                              <tr align="left"  class="listing">
											    <th width="4%">&nbsp;</td>
                                                <th width="55%"><?php echo "<b>".TEXT_TITLE."</b>"; ?></th>
                                                <th width="30%"><?php echo "<b>".TEXT_USER."</b>"; ?></th>
...
?>
											  ...
											  <td align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["nLabelId"]); ?>" class="checkbox">
											   </td>
											   <td>
													<a href="labeltickets.php?id=<?php echo $row["nLabelId"]; ?>&label=<?PHP echo htmlentities(trim_the_string($row["vLabelname"])); ?>&stylename=STYLELABELS&styleminus=minus11&styleplus=plus11&" class="listing"><?PHP echo htmlentities(trim_the_string($row["vLabelname"])); ?></a>
                                                </td>
												
                                                <td><?php echo($row["vLogin"] == "")?"Administrator":$row['vLogin']; ?></td>
                                                <td width="6%"><a href="editlabels.php?id=<?php echo $row["nLabelId"]; ?>&stylename=STYLELABELS&styleminus=minus11&styleplus=plus11&"><img src="././../images/edit.gif" width="13" height="13" border="0" title="<?php echo TEXT_TITLE_EDIT ?>"></a></td>
                                                <td width="5%"><a href="javascript:deleted('<?php echo $row["nLabelId"]; ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_TITLE_DELETE ?>"></a></td>
                                              </tr>
<?php
$cnt++;
}
...
?>
													<table cellpadding="0" cellspacing="0" border="0" width="100%">
														<tr class="whitebasic">
															...
															<div class="pagination_info"><?php echo($navigate[1] ."&nbsp;".TEXT_OF."&nbsp;".$totalrows."&nbsp;" .TEXT_RESULTS ); ?>
															</div></td>
