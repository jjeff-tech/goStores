...
// +----------------------------------------------------------------------+
	...
	if($_POST["postback"] == "D") {
	   	$sql = "Delete from sptbl_cannedmessages where nMsgId='" . addslashes($var_id) . "'";
	...
	if($_POST["postback"] == "U") {
		...
		$var_status = $_POST["rdSts"];
	   	$sql = "Update sptbl_cannedmessages SET vTitle='".addslashes($var_ttl)."',vDescription='".addslashes($var_des)."',vStatus='".addslashes($var_status)."' where nMsgId='" . addslashes($var_id) . "'";
		executeQuery($sql,$conn);
		header("location:cannedmessages.php?mt=y&stylename=STYLECHAT&styleminus=minus12&styleplus=plus12&");
		/*echo("<script>window.location.href=\"pvtmessages.php?mt=y&stylename=STYLEPRIVATEMESSAGES&styleminus=minus2&styleplus=plus2&\"</script>");	
		*/
	...
	$sql = "Select CM.nMsgId,CM.vTitle,CM.vDescription,CM.dDate,CM.vStatus from sptbl_cannedmessages CM where CM.nMsgId='". addslashes($var_id) . "' ";
