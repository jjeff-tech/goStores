...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "" && $var_id != "") {
		$sql = "Select R.nRemId,R.nStaffId,R.vRemTitle,R.tRemDesc,R.dRemAlert,R.dRemPost,S.vStaffname from sptbl_reminders R
		 		inner join sptbl_staffs S on R.nStaffId = S.nStaffId where nRemId='" . addslashes($var_id) . "' ";
	...
	elseif ($_POST["postback"] == "A") {
		...
		if (validateAddition() == true)	{
			$sql = "Insert into sptbl_reminders(nRemId,nStaffId,vRemTitle,tRemDesc,dRemAlert,dRemPost) 
					Values('','$var_staffid','" . addslashes($var_title) . "',
					'" . addslashes($var_desc) . "','" . $var_time . "',now()) ";
			executeQuery($sql,$conn);
			
			$var_insert_id = mysql_insert_id($conn);
			
			if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . addslashes(TEXT_ADDITION) . "','Reminders','$var_insert_id',now())";
            executeQuery($sql,$conn);
			}
	...
	elseif ($_POST["postback"] == "D") {
		...
		if (validateDeletion() == true)	{
			...
			$sql = "Delete from sptbl_reminders where nRemId='" . addslashes($var_id) . "' ";
			executeQuery($sql,$conn);
			
			if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Reminders','" . addslashes($var_id) . "',now())";			
			executeQuery($sql,$conn);
			}
	...
	elseif ($_POST["postback"] == "U") {
		...
		if (validateUpdation() == true)	{
			$sql = "Update sptbl_reminders set 
					vRemTitle='" . addslashes($var_title) . "',
					tRemDesc='" . addslashes($var_desc) . "',
					dRemAlert='$var_time' where nRemId='" . addslashes($var_id) . "'";
			executeQuery($sql,$conn);
			
			if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Reminders','" . addslashes($var_id) . "',now())";			
			executeQuery($sql,$conn);
			}
...
?>
<form name="frmReminder" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>">
<div class="content_section">
	...
	<div class="content_section_data">
		...
		<tr>
         ...
         <td width="61%" align="left">
         <input name="txtAlert" type="text" class="comm_input input_width1a" id="txtAlert" size="24" maxlength="20" value="<?php echo htmlentities($var_alert); ?>" style="font-size:11px;width:120px" readonly="true"> 
