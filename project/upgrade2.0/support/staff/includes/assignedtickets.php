...
if($_POST["del"] == "UP") {
	...
	if($updatedtickets >0){
    	$message = $updatedtickets .  " ticket/s updated successfully";
         $flag_msg="class='msg_success'";
    }
    if($notupdate>0){
...
$sql = "Select t.*,rp.nStaffId,rp.nUserId as rpuserid  from sptbl_tickets t left join sptbl_replies rp on (rp.dDate=t.dLastAttempted and rp.nTicketId=t.nTicketId) where t.vDelStatus='0' AND t.nOwner='$var_staffid' AND t.nLabelId=0";
$qryopt="";
...
<form name="frmDetail" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post">
                  ...
                  <table width="100%"  border="0" cellspacing="0" cellpadding="5">
                         <tr>
                            <td colspan="5" align="center">							<select name="cmbDepartment" class="comm_input input_width1" onChange="javascript:changeDepartment();" style="width:200px">
...
<div class="content_section_title">
				<h3><?php echo HEADING_ASSIGNED_TICKETS." ( ".$dept_name." )"; ?></h3>
			</div>
													 ...
													 case "vPriority":
																for($j=0;$j < count($fld_prio);$j++) {
																	...
																	if ($fld_prio[$j][0] == $row[($fld_arr[$i][0])]) {
																		echo ("<td align=\"center\" ><table  width=\"70%\"><tr class=listing><td bgcolor=" . $fld_prio[$j][1] . " align='center'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . $fld_prio[$j][2] . "</a></td></tr></table></td>");
																	}
																}
													 		break;
													 case "dPostDate":
														 echo "<td width='14%'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . date("m-d-Y",strtotime($row[($fld_arr[$i][0])])) . "</a></td>";
														 break;
													 case "nLockStatus":
