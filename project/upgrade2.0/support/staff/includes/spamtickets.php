...
if($var_deptid != ""){
        ...
        if ($pflag == true) {
                $qryopt .= " AND nDeptId = '" . addslashes($var_deptid) . "' ";
        }
        else {
...
?>
                                              ...
                                              <?php
											    echo "<td width='80%'><a href=\"spamdetails.php?mt=y&spamticketid=" . $row["nSpamTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" .  htmlentities($row['vTitle']) ."</a></td>";
											  	echo "<td width='20%'><a href=\"spamdetails.php?mt=y&spamticketid=" . $row["nSpamTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" .  date("m-d-Y",strtotime(htmlentities($row['dPostDate']))) ."</a></td>";
											  ?>
                                            </tr>
<?php
$cntr++;
