...
if($_POST["del"] == "UP") {
	...
	if($updatedtickets >0){
    	$message = "<font color=\"red\">" . $updatedtickets .  " ticket/s updated successfully.</font><br>";
    }
    if($notupdate>0){
    $message .= "<font color=\"red\">" . $notupdate . " of the selected ticket/s cannot be updated!.</font>";
    }
}
                  ...
                  <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      ...
                      <td class="pagecolor"><table width="100%"  border="0" cellspacing="0" cellpadding="5">
                         <tr>
                            <td colspan="5" align="center">							<select name="cmbDepartment" class="textbox" onChange="javascript:changeDepartment();">
                          ...
                          <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="column1">
							<tr>
                              <td class="whitebasic"><table width="100%"  border="0" cellspacing="0" cellpadding="0" class="whitebasic">
                                  <tr>
...
$navigate = pageBrowser($totalrows,10,$var_maxposts,"&id=$var_id&label=$var_label_name&val=$var_orderby&sorttype=$var_sorttype&pagenum=yes&mt=y&cmbDepartment=$var_deptid&",$var_numBegin,$var_start,$var_begin,$var_num);
$_SESSION["sess_backreplyurl"]="";
$var_back="./assignedtickets.php?mt=y&begin=" . $var_begin . "&num=" . $var_num . "&numBegin=" . $var_numBegin . "&start=$var_start&&cmbSearch=" . $var_cmbSearch . "&txtSearch=" . urlencode($var_search) . "&cmbDepartment=" . urlencode($var_deptid) . "&";
													 ...
													 case "vPriority":
																for($j=0;$j < count($fld_prio);$j++) {
																	...
																	if ($fld_prio[$j][0] == $row[($fld_arr[$i][0])]) {
																		echo ("<td align=\"center\" ><table  width=\"70%\"><tr class=listing><td bgcolor=" . $fld_prio[$j][1] . " align='center'><a href=\"viewticket.php?mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . $fld_prio[$j][2] . "</a></td></tr></table></td>");
																	}
																}
													 		break;
													 case "dPostDate":
														 ...
														 break;
													 case "nLockStatus":
													 		 echo "<td width='3%'><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" . (($row[($fld_arr[$i][0])] == "1")?TEXT_LOCK_YES:TEXT_LOCK_NO) . "</a></td>";
															 break;
													case "vRefNo":
														...
														break;
													case "vTitle":
													  	echo "<td width=48%><span style=\"width:100%;height:100%;\" id=\"link".$count."\" ><a href=\"viewticket.php?limitval=$limitstart&mt=y&tk=" . $row["nTicketId"] . "&us=" . $row["nUserId"] . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus\" class='listing'>" .  htmlentities($row[($fld_arr[$i][0])]) ."</span><br>".$arrayDept[$row["nDeptId"]['name']].  "</a></td>";
													...
													<table cellpadding="0" cellspacing="0" border="0" width="100%">
														<tr class="whitebasic">
		 												  <td colspan="2" align=right>
															 <table width="54%" border="0" align=center cellpadding="0" cellspacing="1" class="column1">
															   <tr class="listing">
															     <td width="26%" class="listing"><?php echo "<b>".TEXT_ACTION_TICKETS."</b>"; ?></td>
																 <td width="27%" align=left class="listing"><?php echo "<b>".TEXT_CHANGESTATUS_TICKETS."</b>"; ?></td>
																 <td width="28%" align=left class="listing"><?php echo "<b>".TEXT_CHANGELABEL_TICKETS."</b>"; ?></td>
																 <td width="19%" rowspan="2" align="center"><a href="javascript:clickUpdate(0);"  class="listing" style="text-decoration:none;"><img src='./../languages/<?php echo $_SESSION['sess_language'];?>/images/go.gif' border=0></a></td>
															   </tr>
																   <tr class="listing">
																	  <td>
