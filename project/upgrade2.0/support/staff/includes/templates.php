...
if ($_POST["postback"] == "D") {
	if (validateDeletion(addslashes($_POST["id"])) == true) {
	    $sql = "delete from  sptbl_templates  where nTemplateId  ='" . addslashes($_POST["id"]) . "'";
		//echo "sql==$sql";
	    executeQuery($sql,$conn);
		
	   //Insert the actionlog
	   if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Templates','" . addslashes($_POST["id"]) . "',now())";					
			executeQuery($sql,$conn);
		}
...
elseif ($_POST["postback"] == "DA") {
	...
	if (validateDeletion($var_list) == true) {
		...
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Templates','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		$var_message = MESSAGE_RECORD_DELETED;
...
if($var_search != ""){
	if($var_cmbSearch == "title"){
	        $qryopt .= " AND st.vTemplateTitle like '" . addslashes($var_search) . "%'";
	}elseif($var_cmbSearch == "date"){
	      
			$qryopt .= " AND date_format(st.dDate,'%m-%d-%Y %H:%i:%s') like '" . addslashes($var_search) . "%'";
	}
}
...
<form name="frmTemplate" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post">
