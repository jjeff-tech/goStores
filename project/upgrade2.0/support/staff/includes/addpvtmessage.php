	...
	if ($_POST["postback"] == "SM") {
	    if(trim($_POST['tosave'])=="" or trim($_POST['txtTitle'])=="" or trim($_POST['txtDesc'])==""){
		  $var_message =  MESSAGE_RECORD_ERROR;
			...
			$var_title = addslashes(trim($_POST["txtTitle"]));
			$var_desc = addslashes(trim($_POST["txtDesc"]));
			...
			for($i=0;$i<count($arr_userid);$i++) {
				...
				if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . addslashes(TEXT_ADDITION) . "','PrivateMessages','" . addslashes($var_new_id) . "',now())";
				executeQuery($sql,$conn);
				}
				
				//notify on pvt message - added on December 27, 2005 
				$sql = "Select vMail,nNotifyPvtMsg from sptbl_staffs where nStaffId='" . addslashes($arr_userid[$i]) . "'";
