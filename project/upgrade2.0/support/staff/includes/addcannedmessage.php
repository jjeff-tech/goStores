...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "A") {
		...
		if (validateAddition() == true) {
				...
				$sql .= " Values('','" . addslashes($var_title) . "','" . addslashes($var_description). "','" . addslashes($var_status) . "','" . addslashes($var_staffid) . "',
						'" . addslashes($var_date) . "')";
				...
				if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . addslashes(TEXT_ADDITION) . "','Canned Message','$var_insert_id',now())";
				executeQuery($sql,$conn);
				}
