...
//Newly Addedon 100609
    $sql_stf = "Select vStaffname from sptbl_staffs where nStaffId='".addslashes($_SESSION['sess_staffid'])."'";
...
//end
	...
	$sql = "Select nTicketId from sptbl_tickets where vStatus='open' and nDeptId IN (".$_SESSION['departmentids'].")";
	$var_cntopen  = mysql_num_rows(executeSelect($sql,$conn));
	$sql = "Select nTicketId from sptbl_tickets where vStatus='closed' and nDeptId IN (".$_SESSION['departmentids'].")";
	$var_cntclosed  = mysql_num_rows(executeSelect($sql,$conn));
	$sql = "Select nTicketId from sptbl_tickets where vStatus='escalated' and nDeptId IN (".$_SESSION['departmentids'].")";
	$var_cntescalated  = mysql_num_rows(executeSelect($sql,$conn));
	$sql = "Select nTicketId from sptbl_tickets where nDeptId IN (".$_SESSION['departmentids'].")";
...
if (mysql_numrows($rs) > 0){
         ...
         $vcontent.="['javascript:showDetails(\"".$row['nNewsId']."\");','".addslashes(htmlentities($row['vTitle']))."','_self']";
         $xxx=1;
