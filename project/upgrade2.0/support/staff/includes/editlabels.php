...
// +----------------------------------------------------------------------+
	...
	elseif ($_POST["postback"] == "A") {
		...
		if (validateAddition() == true and $dup_flag==0) {
			...
			if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . addslashes(TEXT_ADDITION) . "','Labels','" . addslashes($var_insert_id) . "',now())";
				executeQuery($sql,$conn);
			}
