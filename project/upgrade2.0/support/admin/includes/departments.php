...
if ($_POST["postback"] == "D") {
	if (validateDeletion(addslashes($_POST["id"])) == true) {
		$qry="select * from sptbl_depts where nDeptId='".addslashes($_POST["id"])."'";
		
		 $rsgetdept = mysql_query($qry);
		$deptrow=mysql_fetch_array($rsgetdept);
		$oldparentid=$deptrow['nDeptParent'];

		$sql = "delete from  sptbl_staffdept  where nDeptId='".addslashes($_POST["id"])."'";
		executeQuery($sql,$conn);
	
	    $sql = "delete from  sptbl_depts  where nDeptId='" . addslashes($_POST["id"]) . "'";
		executeQuery($sql,$conn);
...
elseif ($_POST["postback"] == "DA") {
	...
	for($i=0;$i<count($_POST["chk"]);$i++) {
		...
		$qry="select * from sptbl_depts where nDeptId='".addslashes($_POST["chk"][$i])."'";
		$rsgetdept = mysql_query($qry);
		$deptrow=mysql_fetch_array($rsgetdept);
	...
	if (validateDeletion($var_list) == true) {
		...
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Department','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		 //assign staff dept where parent dept is leaf
...
if($var_search != ""){
	if($var_cmbSearch == "name"){
			...
			$qryopt .= " AND d.vDeptMail like '" . addslashes($var_search) . "%'";
	}
}
