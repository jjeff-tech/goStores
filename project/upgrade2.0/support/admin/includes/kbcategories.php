...
if ($_POST["postback"] == "D") {
	if (isCategoryDeletable(addslashes($_POST["id"]))) {
	    $sql = "delete from  sptbl_categories  where nCatId='" . addslashes($_POST["id"]) . "'";
		//echo $sql;
		
		executeQuery($sql,$conn);
	   //Insert the actionlog
	   if(logActivity()) {
		$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Categories','" . addslashes($_POST["id"]) . "',now())";					
		executeQuery($sql,$conn);
		}
...
elseif ($_POST["postback"] == "DA") {
	...
	//echo $var_list."<br>";
	if (isCategoryDeletable($var_list) == true) {
		$sql = "delete from  sptbl_categories  where nCatId  IN(" . $var_list . ")";
		executeQuery($sql,$conn);
		//Insert the actionlog
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Categories','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		$var_message = MESSAGE_RECORD_DELETED;
...
function isCategoryDeletable($categorylist) {
	global $conn;
	   $sql="select nCatId from sptbl_categories where nParentId IN($categorylist)";
	   $rs = executeSelect($sql,$conn);
		if(mysql_num_rows($rs)>0){
			  return false;
		}else{
		   $sqlkbcheck="select nCatId from sptbl_kb where nCatId IN($categorylist)";
		   $rs1 = executeSelect($sqlkbcheck,$conn);
...
$sql = " SELECT d.nDeptId,d.vDeptDesc,d.vDeptMail,c.nCompId,c.vCompName, ca.vCatDesc, ca.nCatId ";
$sql .=" FROM sptbl_depts as d,sptbl_companies as c, sptbl_categories as ca";
$sql .=" WHERE d.nCompId=c.nCompId AND ca.nDeptId = d.nDeptId ";
...
if($var_search != ""){
	if($var_cmbSearch == "name"){
			...
			$qryopt .= " AND d.vDeptDesc like '" . addslashes($var_search) . "%'";
	}elseif($var_cmbSearch == "cat"){
			$qryopt .= " AND ca.vCatDesc like '" . addslashes($var_search) . "%'";
	}
}
...
<div class="content_section">
			<div class="content_section_title">
				<h3><?php echo HEADING_CATEGORY_DETAILS ?></h3>
			</div>
									    ...
									    <tr>
                                          <td class="whitebasic" ><table width="100%"  border="0" cellpadding="2" cellspacing="0" class="list_tbl" >
                                              <tr align="left"  class="listing">
                                                ...
                                                <th width="23%"><?php echo "<b>".TEXT_DEPARTMENT."</b>"; ?></th>
                                                <th width="27%"><?php echo "<b>".TEXT_CATEGORY."</b>"; ?></th>
...
?>
                                              ...
                                              <tr align="left"  class="whitebasic">
											    <td align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["nCatId"]); ?>" class="checkbox"></td>
                                                ...
                                                <td width="7%" align="center"><a href="editkbcategory.php?id=<?php echo $row["nCatId"]; ?>&stylename=STYLEKNOWLEDGEBASE&styleminus=minus6&styleplus=plus6&"><img src="././../images/edit.gif" width="13" height="13" border="0" title="<?php echo TEXT_EDIT_CATEGORY; ?>"></a></td>
                                                <td width="7%" align="center"><a href="javascript:deleted('<?php echo $row["nCatId"]; ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_DELETE_CATEGORY; ?>"></a></td>
                                              </tr>
<?php
$cnt++;
}
