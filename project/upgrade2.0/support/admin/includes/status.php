...
if ($_POST["postback"] == "D") {
        if (validateDeletion(addslashes($_POST["id"]),0))  {
            $sql = "delete from  sptbl_lookup  where vLookUpValue='" . addslashes($_POST["id"]) . "' and vLookUpName='ExtraStatus'";
                executeQuery($sql,$conn);
           //Insert the actionlog
		   	if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Lookup/ExtraStatus','" . addslashes($_POST["id"]) . "',now())";
                executeQuery($sql,$conn);
			}	
...
elseif ($_POST["postback"] == "DA") {
        ...
        if (validateDeletion($var_list,1) == true and $var_list!="") {
						...
						if(logActivity()) {
							for($i=0;$i<count($_POST["chk"]);$i++) {
									$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Lookup/ExtraStatus','" . addslashes($_POST["chk"][$i]) . "',now())";
									executeQuery($sql,$conn);
							}
						}
                 ...
                 if (validateAddition(addslashes($_POST["txtExtraStatus"]))) {

                                $sql = "Insert into sptbl_lookup(nLookUpId,vLookUpName,vLookUpValue) values('','ExtraStatus','".addslashes($_POST["txtExtraStatus"])."')";
                                executeQuery($sql,$conn);
                                //Insert the actionlog
								if(logActivity()) {
                                 $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Lookup/ExtraStatus','" . addslashes($_POST["txtExtraStatus"]) . "',now())";
                                 executeQuery($sql,$conn);
								 }
...
<form name="frmExtraStatus" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post">
...
?>
                                              ...
                                              <tr align="left"  class="listingmaintext"><td align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["vLookUpValue"]); ?>" class="checkbox" >
                                                ...
                                                <td width="15%"  align="center"><a href="javascript:deleted('<?php echo addslashes($row["vLookUpValue"]); ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_DELETE_STATUS?>"></a></td>
                                              </tr>
<?php
$cnt++;
}
