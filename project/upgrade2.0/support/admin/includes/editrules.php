...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "" && $var_id != "") {
		$sql = "select d.vDeptDesc,r.vRuleName,c.vCompName,sd.*,s.vStaffName,r.nSearchTitle,r.nSearchBody,r.vSearchWords,c.nCompId
				...
				where sd.nDeptId= r.nDeptId and r.nStaffId= s.nStaffId and sd.nStaffId='".addslashes($var_stid)."' and r.nRuleId='".addslashes($var_id)."'";
...
*/
			if (validateRulename() == true) {
			   if(validateKeyword() == true){
					...
					$sql .= ") Values('','" . addslashes($var_rulename). "','" . addslashes($var_keywords). "','" . addslashes($var_stid). "','" . addslashes($var_parentid) . "',now())";

					executeQuery($sql,$conn);
					$var_insert_id = mysql_insert_id($conn);
					//Insert the actionlog
					if(logActivity()) {
						$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Rules','" . addslashes($var_insert_id) . "',now())";
						executeQuery($sql,$conn);
					}
	...
	elseif ($_POST["postback"] == "D") {
			$sql = "delete from  sptbl_rules where nRuleId='" . addslashes($var_id) . "'";
			executeQuery($sql,$conn);
...
*/			
			...
			$sqlDuplicate = "SELECT nRuleId FROM sptbl_rules WHERE vRuleName='". addslashes($var_rulename) ."' and nDeptId='$var_parentid' and nRuleId !='$var_id'";
			...
			if ($dup_flag==0) {				
						$sql = "Update sptbl_rules set vRuleName='" . addslashes($var_rulename) . "',
								nSearchTitle='" . addslashes($var_searchtitle) . "',
								nSearchBody='" . addslashes($var_searchbody) . "',
								vSearchWords='" . addslashes($var_keywords) . "',
								nStaffId='" . addslashes($var_stid). "',
								nDeptId='" . addslashes($var_parentid). "'
								where nRuleId='" . addslashes($var_id) . "'";
						executeQuery($sql,$conn);
						
					//Insert the actionlog
					if(logActivity()) {
						$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Rules','" . addslashes($var_id) . "',now())";			
						executeQuery($sql,$conn);
					}
...
// to check duplicate keywords
			...
			$sqlDuplicate = "select distinct r.vSearchWords from sptbl_rules r,sptbl_staffdept sd,sptbl_staffs s
				left join sptbl_depts as d on d.nDeptId= r.nDeptId 
				left join sptbl_companies c on c.nCompId = d.nCompId
				where sd.nDeptId= r.nDeptId and r.nStaffId= s.nStaffId and r.nDeptId='".addslashes($var_parentid)."'";
                        */
                        $sqlDuplicate = "select distinct r.vSearchWords from sptbl_rules r
                                            ...
                                            left join sptbl_depts as d on d.nDeptId= r.nDeptId
                                            left join sptbl_companies c on c.nCompId = d.nCompId
                                            where r.nDeptId='".addslashes($var_parentid)."'";
	...
	function validateRulename() 
	{
		...
		if (trim($_POST["cmbParentDepartment"]) <= 0 || trim($_POST["txtKeywords"]) =="" || trim($_POST["cmbCompany"])<=0 ||
			...
			//check duplicate rule name
			$dup_flag=0;
			$sqlDuplicate = "SELECT nRuleId FROM sptbl_rules WHERE vRuleName='". addslashes($var_rulename) ."' and nDeptId='$var_parentid'";
			$rsDuplicate  = executeSelect($sqlDuplicate,$conn);
...
?>
<form name="frmRules" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>">
