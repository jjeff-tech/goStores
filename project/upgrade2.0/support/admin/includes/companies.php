...
if ($_POST["postback"] == "D") {
	if (validateDeletion(addslashes($_POST["id"])) == true) {
		...
		$sql = "Update sptbl_companies set vDelStatus = '1' where nCompId = '" . addslashes($_POST["id"]) . "'";
		executeSelect($sql,$conn);
		
		//Insert the actionlog
		if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Company','" . addslashes($_POST["id"]) . "',now())";			
...
elseif ($_POST["postback"] == "DA") {
	...
	if (validateDeletion($var_list) == true) {
		...
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Company','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		$var_message = MESSAGE_RECORD_DELETED;
...
if($var_search != ""){
	if($var_cmbSearch == "name"){
			$qryopt .= " AND vCompName like '" . addslashes($var_search) . "%'";
	}elseif($var_cmbSearch == "city"){
			$qryopt .= " AND vCompCity like '" . addslashes($var_search) . "%'";
	}
        elseif($var_cmbSearch == "email"){
			$qryopt .= " AND vCompMail like '" . addslashes($var_search) . "%'";
	}
}
