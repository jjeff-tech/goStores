...
if ($_POST["postback"] == "D") {
        if (validateDeletion(addslashes($_POST["id"])) == true) {
                ...
                $sql = "delete from sptbl_pvtmessages where nPMId = '" . addslashes($_POST["id"]) . "'";
                executeSelect($sql,$conn);

                //Insert the actionlog
				if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','PrivateMessages','" . addslashes($_POST["id"]) . "',now())";
...
elseif ($_POST["postback"] == "DA") {
        ...
        if (validateDeletion($var_list) == true) {
				...
				if(logActivity()) {
					for($i=0;$i<count($_POST["chk"]);$i++) {
							$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','PrivateMessages','" . addslashes($_POST["chk"][$i]) . "',now())";
							executeQuery($sql,$conn);
					}
				}
                $var_message = MESSAGE_RECORD_DELETED;
...
$sql = "Select PM.nPMId,PM.vPMTitle,PM.dDate,PM.vStatus,S.vStaffName as 'FromName',S1.vStaffName as 'ToName'
                 ...
                 sptbl_staffs S1 on PM.nToStaffId = S1.nStaffId ";
...
if($var_search != ""){
        if($var_cmbSearch == "fromname"){
                        $qryopt .= " WHERE S.vStaffName like '" . addslashes($var_search) . "%'";
        }elseif($var_cmbSearch == "toname"){
                        $qryopt .= " WHERE S1.vStaffName like '" . addslashes($var_search) . "%'";
        }elseif($var_cmbSearch == "title"){
                        $qryopt .= " WHERE PM.vPMTitle like '" . addslashes($var_search) . "%'";
        }
}
                                    ...
                                    <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
											...
											<table width="100%" cellpadding="0" cellspacing="0" border="0">
													...
													<tr>
														<td width="64%" align="right" class="whitebasic">
														<div class="content_search_container" style="background-color:#ffffff; ">
															...
															<div class="left rightmargin">
																...
																<option value="toname" <?php echo(($var_cmbSearch == "toname")?"Selected":""); ?>><?php echo TEXT_TO ?></option>
                                          ...
                                          <td class="whitebasic" >
											  ...
											  <th>&nbsp;</th>
                                                <th width="21%"><?php echo "<b>".TEXT_FROM."</b>"; ?></th>
                                                <th width="17%"><?php echo "<b>".TEXT_TO."</b>"; ?></th>
                                                <th width="21%"><?php echo "<b>".TEXT_TITLE."</b>"; ?></th>
                                                <th width="14%"><?php echo "<b>".TEXT_DATE."</b>"; ?></th>
                                                <th width="13%"><?php echo "<b>".TEXT_STATUS."</b>";; ?></td>
...
?>
											  ...
											  <td width="4%" align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["nPMId"]); ?>" class="checkbox">
                                              <td width="21%"><?PHP echo htmlentities($row["FromName"]); ?></td>
                                                <td width="17%"><?php echo htmlentities($row["ToName"]); ?></td>
                                                <td width="21%"><?php echo htmlentities($row["vPMTitle"]); ?></td>
                                                <td width="14%"><?php echo date("m-d-Y",strtotime($row["dDate"])); ?></td>
                                                <td width="13%"><?php echo (($row["vStatus"] == "o")?"New":"Viewed") ?></td>
                                                <td width="5%"><a href="vwpvtmessage.php?id=<?php echo $row["nPMId"]; ?>&stylename=STYLEPRIVATEMESSAGES&styleminus=minus13&styleplus=plus13&ret=a&"><img src="././../images/view.gif" width="13" height="13" border="0" title="<?php echo TEXT_VIEW_PRIVATE_MESSAGE?>"></a></td>
                                                <td width="5%"><a href="javascript:deleted('<?php echo $row["nPMId"]; ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_DELETE_PRIVATE_MESSAGE?>"></a></td>
                                              </tr>
<?php
$cnt++;
}
