...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "" && $var_id != "") {
		...
		$sql .= "vBanned,vDelStatus from sptbl_users where nUserId = '" . addslashes($var_id) . "'";
	...
	elseif ($_POST["postback"] == "D") {
			...
			if (validateDeletion($var_id) == true) {
				$sql = "Update sptbl_users set vDelStatus = '2' where nUserId='" . addslashes($var_id) . "'";
				executeQuery($sql,$conn);
	...
	function validateAddition() 
	{
		...
		elseif(!isValidEmail(trim($_POST["txtEmail"]))){
			...
			if (mysql_num_rows(executeSelect($sql,$conn)) <= 0) {
				...
				$sql = "Select nUserId from sptbl_users where vLogin='" . addslashes(trim($_POST["txtUserLogin"])) . "' AND vDelStatus = 0";
				if (mysql_num_rows(executeSelect($sql,$conn)) > 0) {
