...
// +----------------------------------------------------------------------+
	...
	elseif ($_POST["postback"] == "A") { // Inserting New Rule
		...
		if (validateAddition($ruleName,$var_message) == true) {
			...
			$sql .= "nStatus) Values('','" . addslashes($ruleName) . "',
					'" . addslashes($comapny). "','" . addslashes($dept) . "','" . addslashes($Time_settings) . "',
					'" . addslashes($Count_settings) . "','" . addslashes($txtTime) . "','" . addslashes($txtCount) . "',
					'" . addslashes($staff) . "','0')";
	...
	elseif ($_POST["postback"] == "U") { // Updating Rule
			...
			if (validateUpdation($ruleName,$var_message) == true) {
				$sql = "Update sptbl_escalationrules set vRuleName='" . addslashes($ruleName) . "',
					nCompId='" . addslashes($comapny) . "',
					nDeptId='" . addslashes($dept). "',
					eRespTimeSetting='" . addslashes($Time_settings) . "',
					eRespCountSetting='" . addslashes($Count_settings) . "',
					nResponseTime='" . addslashes($txtTime) . "',
					nResponseCount='" . addslashes($txtCount) . "',
					nStaffId='" . addslashes($staff). "'
	...
	function validateAddition($ruleName,&$var_message)
	{
		...
		if (trim($_POST["txtRuleName"]) == "" || trim($_POST["txtCompany"]) == "" || trim($_POST["txtDept"]) == "" || trim($_POST["txtStaff"]) == "") {
			...
			$sql = "Select vRuleName from sptbl_escalationrules where vRuleName='" . addslashes(trim($_POST["txtRuleName"])) . "'";
	...
	function validateUpdation($ruleName,&$var_message)
	{
		...
		$sql = "Select nERId from sptbl_escalationrules Where vRuleName='" . addslashes(trim($_POST["txtRuleName"])) . "' AND nERId !='" . addslashes($var_id) . "' ";
		if (mysql_num_rows(executeSelect($sql,$conn)) > 0) {
			$var_message = MESSAGE_RECORD_DUPILCATION ;
                        $flag_msg    = 'class="msg_error"';
			return false;
