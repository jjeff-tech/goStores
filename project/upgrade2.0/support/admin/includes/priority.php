...
if ($_POST["postback"] == "" && $var_id != "") {
	              $sql = "Select * from sptbl_priorities where nPriorityId='".addslashes($var_id)."'";
        ...
        if (validateDeletion(addslashes($_POST["id"])) and addslashes($_POST["id"]) !="1")  {
            $sql = "delete from  sptbl_priorities where nPriorityValue !='0' and  nPriorityId='" . addslashes($_POST["id"]) . "'";
		   ...
		   if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Priority','" . addslashes($_POST["id"]) . "',now())";
                executeQuery($sql,$conn);
			}
			// it is to update tickets table 
				$sql = "update sptbl_tickets set vPriority='0' where vPriority='" . addslashes($_POST["id"]) . "'";
				executeQuery($sql,$conn);
...
elseif ($_POST["postback"] == "DA") {
        ...
        if (validateDeletion($var_list) == true and $var_list!="") {
						...
						if(logActivity()) {
							for($i=0;$i<count($_POST["chk"]);$i++) {
									$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Priority','" . addslashes($_POST["chk"][$i]) . "',now())";
									executeQuery($sql,$conn);
							}
						}
			   ...
			   default:
                ...
                // Upload icon  ends
                 ...
                 if (validateAddition()==true) {
			                 ...
			                 $sql .= " Values('','" . addslashes($var_prtyvalue). "','" . addslashes($var_prtydesc) . "','" . addslashes($var_prtycolor) . "','". mysql_real_escape_string($txticon_Url)  . "')";
			                 executeQuery($sql,$conn);
			                 $var_insert_id = mysql_insert_id($conn);
			                 //Insert the actionlog
							 if(logActivity()) {
			                 $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Priority','" . addslashes($var_insert_id) . "',now())";			
			                 executeQuery($sql,$conn);
							 }
...
//				         break;
    ...
    if (validateUpdation($var_prtyid)==true) {
		if($var_oldpriority == 0)
		 ...
		 $sql = "update sptbl_priorities set nPriorityValue='".addslashes($var_prtyvalue). "',";
		 $sql .="vPriorityDesc='". addslashes($var_prtydesc) . "',vTicketColor='".addslashes($var_prtycolor) . "' ";
                 if($txticon_Url != ""){
		 ...
		 $sql .= " where nPriorityId='" . addslashes($var_prtyid) . "'";
		// echo "sql==$sql";exit;
		 executeQuery($sql,$conn);
		 if(logActivity()) {
			 $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Priority','" . addslashes($var_prtyid) . "',now())";			
			 executeQuery($sql,$conn);
		 }
// it is to update tickets table 
		 $sql = "update sptbl_tickets set vPriority='".addslashes($var_prtyvalue). "' where vPriority='" . addslashes($var_oldpriority) . "'";
		 executeQuery($sql,$conn);
...
function validateUpdation($varpid) {
			 ...
			 $sql="Select * from sptbl_priorities where (nPriorityValue='".addslashes($_POST['cmbValue'])."'";
			 $sql .=" or vPriorityDesc='".addslashes($_POST['txtExtraPrty'])."') and nPriorityId !='".addslashes($varpid)."'";
...
function validateAddition() {
			 ...
			 $sql="Select * from sptbl_priorities where nPriorityValue='".addslashes($_POST['cmbValue'])."'";
			 $sql .=" or vPriorityDesc='".addslashes($_POST['txtExtraPrty'])."'";
...
if($var_search != "" and $var_cmbSearch == "name" ){
                ...
                $qryopt .= " and vPriorityDesc like '" . addslashes($var_search) . "%'";
...
<form name="frmExtraPriority" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
...
<!-- Color picker ends-->
                              ...
                              <tr align="center" class="whitebasic" >
                                  ...
                                  <td  align="left">
                                        ...
                                        if($txticon_Url  != ""){
                                        ?>
                                        <img alt="icon" src="<?php echo $txticon_Url;?>" height="50" width="50">
                                        <?php
                                        }
                                        ?>
