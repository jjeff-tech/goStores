...
if ($_POST["postback"] == "D") {
	if (validateDeletion(addslashes($_POST["id"])) == true and addslashes($_POST["id"]) !="1") {
		...
		$sql = "Update sptbl_staffs set vDelStatus = '1' where nStaffId = '" . addslashes($_POST["id"]) . "'";
		executeSelect($sql,$conn);
		
		//Insert the actionlog
		if(logActivity()) {
			$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Staff','" . addslashes($_POST["id"]) . "',now())";			
...
elseif ($_POST["postback"] == "DA") {
	...
	if (validateDeletion($var_list) == true and $var_list !="") {
		...
		if(logActivity()) {
			for($i=0;$i<count($_POST["chk"]);$i++) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Staff','" . addslashes($_POST["chk"][$i]) . "',now())";			
				executeQuery($sql,$conn);
			}	
		}
		$var_message = MESSAGE_RECORD_DELETED;
...
$sql = "Select * from sptbl_staffs where vDelStatus='0'";
...
if($var_search != ""){
	if($var_cmbSearch == "name"){
			$qryopt .= " AND vStaffname like '" . addslashes($var_search) . "%'";
	}elseif($var_cmbSearch == "mail"){
			$qryopt .= " AND vMail like '" . addslashes($var_search) . "%'";
	}
}
