  ...
  if($wr== TEXT_WRITE_PERMISSION_AVAILABLE){
    ...
    $recom=$wr;
        ...
        if ($_POST["postback"] == "" && $var_id != "") {
                ...
                $sql = "Select vLangCode,vLangDesc from sptbl_lang where vlangCode = '" . addslashes($var_id) . "'";
		       ...
		       if($user_flag==1 or $staff_flag==1 or $admin_flag==1 or $parser_flag==1){
				...
				if(isValidlangcode($var_langCode) and isValidlangcode($var_langDesc)){
		                ...
		                if (validateAddition($var_langCode,$var_langDesc) == true) {
								...
								if($errorinfiles==0){
				                        $sql = "Insert into sptbl_lang(vLangCode,vlangdesc) Values('" . addslashes($var_langCode) . "','" . addslashes($var_langDesc)."')";
        ...
        elseif ($_POST["postback"] == "D") {
                ...
                if (validateDeletion($var_id) == true and $var_id!="en") {
                        $sql = "Delete from  sptbl_lang  where vLangCode='" . addslashes($var_id) . "'";
                        executeQuery($sql,$conn);

                        //Insert the actionlog
						if(logActivity()) {
                        $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Language','" . addslashes($var_id) . "',now())";
                        executeQuery($sql,$conn);
						}
        ...
        elseif ($_POST["postback"] == "U") {
                        ...
                        if (validateUpdation($var_langCode, $var_langDesc) == true  and $var_langCode !="en") {
                           $sql = "Update sptbl_lang set    vLangDesc='" . addslashes($var_langDesc) . "'  where vLangCode='" . addslashes($var_id) . "'";
                                executeQuery($sql,$conn);

                        //Insert the actionlog
						if(logActivity()) {
                        $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Language','" . addslashes($var_id) . "',now())";
                        executeQuery($sql,$conn);
						}
        ...
        function validateAddition($var_langCode,$var_langDesc)
        {
                ...
                If (trim($_POST["txtLangCode"]) == "" || trim($_POST["txtLangDesc"]) == "" )               {
                  ...
                  vLangCode='" . addslashes($var_langCode) ."' or
                  vLangDesc='". addslashes($var_langDesc) ."'";
                ...
                <td>
                    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                                  <tr align="center"  class="listingbtnbar">
                                    <td width="22%">&nbsp;</td>
                                    <td width="16%"><input name="btAdd" type="button" class="comm_btn" value="<?php echo BUTTON_TEXT_ADD ?>" onClick="javascript:add();" <?php echo   $addbuttonenabled?>></td>
