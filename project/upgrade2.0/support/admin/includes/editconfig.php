...
// +----------------------------------------------------------------------+
        ...
        elseif ($_POST["postback"] == "U") {
             ...
             if($var_port !="" && !is_numeric($var_port)){
				 ...
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_USER_DIR) .
				 "'  where vLookUpName = 'AD_USER_DIR'";
				 executeQuery($sql,$conn);
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_HOST) .
				 "'  where vLookUpName = 'AD_HOST'";
				 executeQuery($sql,$conn);
				  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_DOMAIN) .
				 "'  where vLookUpName = 'AD_DOMAIN'";
				 executeQuery($sql,$conn);
				  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_AUTHENTICATION) .
				 "'  where vLookUpName = 'AD_AUTHENTICATION'";
				 executeQuery($sql,$conn);
				  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_USER) .
				 "'  where vLookUpName = 'AD_USER'";
				 executeQuery($sql,$conn);
				  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($AD_PASS) .
				 "'  where vLookUpName = 'AD_PASS'";
				 executeQuery($sql,$conn);
				
				
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_siteURL) .
				 "'  where vLookUpName = 'SiteURL'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_helpDeskURL) . "'
				 where vLookUpName = 'HelpDeskURL'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_defaultLang) . "'
				 where vLookUpName='DefaultLang'";
				
				 executeQuery($sql,$conn);
				 $_SESSION["sess_defaultlang"]=addslashes($var_defaultLang);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_post2PostGap) . "'
				 where vLookUpName='Post2PostGap'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_langChoice) . "'
				 where vLookUpName='LangChoice'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_autoLock) . "'
				 where vLookUpName='AutoLock'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_verifyTemplate) . "'
				 where vLookUpName='VerifyTemplate'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_verifyKB) . "'
				 where vLookUpName='VerifyKB'";
				 executeQuery($sql,$conn);
				 
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_approvelog) .
				 "'  where vLookUpName = 'logactivity'";
				 executeQuery($sql,$conn);
				 
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_emailpiping) .
				 "'  where vLookUpName = 'EmailPiping'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_postticketbeforeregister) .
				 "'  where vLookUpName = 'PostTicketBeforeLogin'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_userauthentication) .
				 "'  where vLookUpName = 'UserAuthenticate'";
				 executeQuery($sql,$conn);
	
				 $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_messagerule) .
				 "'  where vLookUpName = 'MessageRule'";
				 executeQuery($sql,$conn);
				 
				$sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_maxPost) .
				 "'  where vLookUpName = 'MaxPostsPerPage'";
				 executeQuery($sql,$conn);
	
				$sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_messageOrder) .
				 "'  where vLookUpName = 'OldestMessageFirst'";
				 executeQuery($sql,$conn);
				 
				$sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_smtp) .
				 "'  where vLookUpName = 'SMTPSettings'";
				 executeQuery($sql,$conn);
	
				$sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_smtpserver) .
				 "'  where vLookUpName = 'SMTPServer'";
				 executeQuery($sql,$conn);
	
				$sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_port) .
				 "'  where vLookUpName = 'SMTPPort'";
				 executeQuery($sql,$conn);
                               $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_livechat) .
				 "'  where vLookUpName = 'LiveChat'";
				 executeQuery($sql,$conn);
                                  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_ticketmail) .
				 "'  where vLookUpName = 'NewTicketAutoReturnMail'";
				 executeQuery($sql,$conn);
                                  $sql = "Update sptbl_lookup set vLookUpValue='" . addslashes($var_theme) .
				...
				$sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($emailurl) . "'
				 where vLookUpName = 'EmailURL'";
				 executeQuery($sql,$conn);
	
                                  //update css
                                  $sql = "Select vCSSURL from sptbl_css where nCSSId='".addslashes($var_theme)."'";
...
<form name="frmConfig" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>">
<div class="content_section">
		...
		<table width="100%"  border="0">
		  <tr>
			<td width="76%" valign="top">
				 ...
				 <td width="39%" align="left" class="fieldnames"><?php echo TEXT_CON_SITEURL?>&nbsp;<font style="color:#FF0000; font-size:9px">*</font></td>
				 <td width="55%" align="left">
				 <input name="txtSiteURL" type="text" class="comm_input input_width1" id="txtSiteURL" size="30" maxlength="100" value="<?php echo htmlentities($var_siteURL); ?>">
				 </td>
				 </tr>
				 ...
				 <td width="55%" align="left">
				 <input name="txtHelpDeskURL" type="text" class="comm_input input_width1a" id="txtHelpDeskURL" size="30" maxlength="100" value="<?php echo htmlentities($var_helpDeskURL); ?>" style="width:350px">
				 </td>
				 </tr>
				...
				<tr><td colspan="3">&nbsp;</td></tr>
				 ...
				 <td width="55%" align="left">
				 <input name="txtMaxPost" type="text" class="comm_input input_width1" id="txtMaxPost" size="10" maxlength="3" value="<?php echo htmlentities($var_maxPost); ?>" >
				 </td>
				 </tr>
				 ...
				 <select name="cmbPost2PostGap" class="comm_input input_width1a">
				...
				?>
				<select name="cmbDefaultLang" size="1" class="comm_input input_width1a" id="cmbDefaultLang" >
				<?php
		...
		<?php /* modified by roshith on 06-11-2006 */ ?>
				 ...
				 <td width="55%" align="left"  class="fieldnames">
                                    <?php echo makeDropDownList("ddlCSS",getCSSList(),$var_theme, "comm_input input_width1", $properties, "", true ); ?>
				 </td>
				 </tr>
				 ...
				 <td width="55%" align="left"  class="listing">4.2</td>
				 </tr>
				  ...
				  <tr id="ad13">
				 <td width="6%" align="left">&nbsp;</td>
				 <td width="39%" align="left" class="fieldnames"><?php echo TEXT_CURRENT_VERSION;?>&nbsp;</td>
				 <td width="55%" align="left"  class="listing">4.2</td>
				 </tr>
