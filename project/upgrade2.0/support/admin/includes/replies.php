...
//		  	  echo $sqlnext;
		 ...
		 if($validsave =="1"){
		     //insert into personal notes
				  if(trim($var_pntitle)!="" and trim($var_pnmatter)!=""){
				   ...
				   if(logActivity()) {
					   $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Personal Notes','" . addslashes($var_insert_id) . "',now())";
					   executeQuery($sql,$conn);
				   }
				 ...
				 if($var_addtokb=="atokb" and $var_category >0){
				  ...
				  //update ticket fileds
					 ...
					 if($var_autoclock=="1"){
					   ...
					   if(logActivity()) {
						   $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Ticket','" . addslashes($var_tid) . "',now())";
						   executeQuery($sql,$conn);
					   }
		       //send mail to user
  ...
  executeQuery($sql_reply_up,$conn);
					  ...
					  //Insert the actionlog
					  if(logActivity()) {
						   $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Reply','" . addslashes($var_insert_id) . "',now())";
						   executeQuery($sql,$conn);
					  }
		 ...
		 if($validsave =="1"){
		     //insert into personal notes
		       ...
		       //send mail to user
                    ...
                    $sql_reply_up = "update sptbl_replies set tReply='".addslashes($var_replymatter)."',vReplyTime='".addslashes($var_timespent)."',nReplyStatus='".addslashes($var_status)."',dDate='".$now."' where nReplyId='".$var_reply_id."'";
  executeQuery($sql_reply_up,$conn);
					   $var_insert_id = $var_reply_id;
                }
					  //Insert the actionlog
								 ...
								 <tr>
										    ...
										    <td  align="left">
		 									...
		 									<?php                  $sBasePath                      = "../FCKeditor/";
                                                                                                                ...
                                                                                                                $oFCKeditor->Value		       =  $var_replymatter;
...
<br>
                  ...
                  <table width="100%"  border="0" cellspacing="0" cellpadding="0">
				  					<tr>
										<td colspan="3" align="center">
											<div class="ticketreplies_btncontainer">
