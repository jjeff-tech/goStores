...
// +----------------------------------------------------------------------+
	...
	if ($_POST["postback"] == "" && $var_id != "") {
        ...
        $sql .=" where nLabelId ='".addslashes($var_id)."'";
	...
	elseif ($_POST["postback"] == "A") {
			...
			$sql="select *  from sptbl_labels  WHERE   vLabelname ='".addslashes($var_title) . "'";
		...
		if (validateAddition() == true and $dup_flag==0) {
			...
			$sql .= ") Values('','" . addslashes($var_title). "','$var_staffid')" ;
		    executeQuery($sql,$conn);
			$var_insert_id = mysql_insert_id($conn);
			//Insert the actionlog
			if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','Labels','" . addslashes($var_insert_id) . "',now())";			
				executeQuery($sql,$conn);
			}
	...
	elseif ($_POST["postback"] == "D") {
		if (validateDeletion() == true) {
			$sql = "delete from  sptbl_labels  where nLabelId ='" . addslashes($var_id) . "'";
			executeQuery($sql,$conn);
			
			//Insert the actionlog
			if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Labels','" . addslashes($var_id) . "',now())";			
				executeQuery($sql,$conn);
			}
			$var_title= "";
			$var_desc = "";
	...
	elseif ($_POST["postback"] == "U") {
	   $sql = "Select nLabelId from sptbl_labels Where nLabelId='" . addslashes($var_id) . "'";    
		if(mysql_num_rows(executeSelect($sql,$conn)) > 0) {
			...
			if (validateUpdation() == true and $dup_flag==0) {
				 ...
				 $sql = "Update sptbl_labels  set vLabelname ='" . addslashes($var_title) . "'
					     where nLabelId='" . addslashes($var_id) . "'";
						
			executeQuery($sql,$conn);
			//Insert the actionlog
			if(logActivity()) {
				$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','Labels','" . addslashes($var_id) . "',now())";
				executeQuery($sql,$conn);
			}	    
...
<form name="frmLabel" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>">
