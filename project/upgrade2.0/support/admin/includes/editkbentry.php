	...
	if ($_POST["postback"] == "" && $var_id != "") {//if the page is not posted
       ...
       $sqla .= " WHERE k.nKBID = '".addslashes($var_id)."'";
		...
		if(mysql_num_rows(executeSelect($sql,$conn)) > 0) { 
			...
			if(!$error){
				$sql  =  "UPDATE sptbl_kb SET nCatId= '" . addslashes($var_catid) . "', nStaffId = '" . addslashes($_SESSION["sess_staffid"]) . "', vKBTitle='" . addslashes($var_kbtitle) . "', " ;
				$sql .= " tKBDesc = '". addslashes($var_kbdesc) ."', vStatus = '$var_status', vMetaTage_keyword = '". mysql_real_escape_string($var_kbmetatagkeyword) ."' , vMetaTage_desc = '". mysql_real_escape_string($var_kbmetatagdescription) ."'";
