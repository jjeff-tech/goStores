...
if ($_POST["postback"] == "D") {
        if (validateDeletion(addslashes($_POST["id"])) == true and addslashes($_POST["id"]) != "en" and  addslashes($_POST["id"]) !=$_SESSION["sess_defaultlang"]) {
            $sql = "delete from  sptbl_lang  where vLangCode='" . addslashes($_POST["id"]) . "'";
                executeQuery($sql,$conn);
           //Insert the actionlog
		   	if(logActivity()) {
                $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Language','" . addslashes($_POST["id"]) . "',now())";
                executeQuery($sql,$conn);
				}
...
elseif ($_POST["postback"] == "DA") {
        ...
        if (validateDeletion($var_list) == true and $var_list!="") {
						...
						if(logActivity()) {
							for($i=0;$i<count($_POST["chk"]);$i++) {
									$sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','Language','" . addslashes($_POST["chk"][$i]) . "',now())";
									executeQuery($sql,$conn);
							}
						}
...
$sql = "Select vLangCode,vLangDesc from sptbl_lang ";
...
if($var_search != ""){
        if($var_cmbSearch == "code"){
                $qryopt .= " Where vLangCode like '" . addslashes($var_search) . "%'";
        }elseif($var_cmbSearch == "desc"){
                        $qryopt .= " Where vlangDesc like '" . addslashes($var_search) . "%'";
        }
}
...
$sql .= $qryopt . " Order By vLangDesc";

?>

<form name="frmLang" action="<?php echo   $_SERVER['PHP_SELF']?>" method="post">

<div class="content_section">
			<div class="content_section_title">
				<h3> <?php echo TEXT_LANGUAGE_DETAILS ?></h3>
			</div>
										...
										<td width="100%">
											...
											<table width="100%" cellpadding="0" cellspacing="0" border="0">
														...
														<tr>
															...
															<option value="code" <?php echo(($var_cmbSearch == "code" || $var_cmbSearch == "")?"Selected":""); ?>><?php echo TEXT_LANGUAGE_CODE?></option>
															<option value="desc" <?php echo(($var_cmbSearch == "desc")?"Selected":""); ?>><?php echo TEXT_LANGUAGE_DESCRIPTION?></option>
															</select>
															</div>
															<div class="left">
															<input type="text" name="txtSearch" value="<?php echo(htmlentities($var_search)); ?>" class="inputstyle" onKeyPress="if(window.event.keyCode == '13'){ return false; }" style="width:140px">&nbsp;&nbsp;</div>
															<div class="left">
                                          ...
                                          <td class="whitebasic" ><table width="100%"  border="0" cellpadding="2" cellspacing="0" class="list_tbl" >
                                              <tr align="left"  class="listing">
											    <th width="5%">&nbsp;</th>
                                                <th width="55%"><?php echo "<b>".TEXT_LANGUAGE_CODE."</b>"; ?></th>
                                                <th ><?php echo "<b>".TEXT_LANGUAGE_DESCRIPTION."</b>"; ?></th>
                                                <th align="left"><?php echo "<b>".TEXT_ACTION."</b>"; ?></th>
                                              </tr>
<?php
...
$navigate = pageBrowser($totalrows,10,10,"&mt=y&cmbSearch=$var_cmbSearch&txtSearch=" . urlencode($var_search) . "&stylename=$var_stylename&styleminus=$var_styleminus&styleplus=$var_styleplus&",$var_numBegin,$var_start,$var_begin,$var_num);
...
?>
                                              ...
                                              <tr align="left"  class="whitebasic">
											     <td align="center"><input type="checkbox" name="chk[]" id="c<?php echo($cnt); ?>" value="<?php echo($row["vLangCode"]); ?>" class="checkbox" ></td>
												 <td><?PHP echo htmlentities(trim_the_string($row["vLangCode"])); ?></td>
                                                <td><?php echo htmlentities(trim_the_string($row["vLangDesc"])); ?></td>
                                                <!--<td width="6%"><a href="editlang.php?id=<?php echo $row["vLangCode"]; ?>&stylename=<?php echo($var_stylename); ?>&styleminus=<?php echo($var_styleminus); ?>&styleplus=<?php echo($var_styleplus); ?>&"><img src="././../images/edit.gif" width="13" height="13" border="0" title="<?php echo TEXT_EDIT_LANGUAGE?>"></a></td> -->
												<td width="21%" class="whitebasic">
												 <?php 												 
												 if($row["vLangCode"] !="en" and $row["vLangCode"] !=$_SESSION["sess_defaultlang"]){?>
                                                <a href="javascript:deleted('<?php echo $row["vLangCode"]; ?>');"><img src="././../images/delete.gif" width="13" height="13" border="0" title="<?php echo TEXT_DELETE_LANGUAGE ?>"></a>
												<?php } ?>
                                                </td>
												</tr>
<?php
                        ...
                        <td class="pagecolor"><table width="100%"  border="0" cellspacing="0" cellpadding="5">
                              <tr>
                                <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                                  <tr align="right" class="listingbtnbar">
                                    <td align="center">
