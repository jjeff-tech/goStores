...
// +----------------------------------------------------------------------+
        ...
        elseif ($_POST["postback"] == "U") {
			...
			if(checkLookupDetails($arr,$returnList) == true) {
				 ...
				 vLookUpValue='" . addslashes($var_mailAdmin) . "'
				 where vLookUpName = 'MailAdmin'";
				 executeQuery($sql,$conn);

				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailTechnical) . "'
				 where vLookUpName = 'MailTechnical'";
				 executeQuery($sql,$conn);

				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailEscalation) . "'
				 where vLookUpName='MailEscalation'";
				 executeQuery($sql,$conn);

				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailFromName) . "'
				 where vLookUpName='MailFromName'";
				 executeQuery($sql,$conn);


				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailFromMail) . "'
				 where vLookUpName='MailFromMail'";
				 executeQuery($sql,$conn);


				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailReplyName) . "'
				 where vLookUpName='MailReplyName'";
				 executeQuery($sql,$conn);


				 $sql = "Update sptbl_lookup set
				 vLookUpValue='" . addslashes($var_mailReplyMail) . "'
		...
		foreach($arr_lookup as $key=>$value) {
			...
			if($sub_flag == true) {
				$sql = "Select * from dummy d
					...
					Left JOIN sptbl_staffs s on (d.num=1 AND s.vMail='" . addslashes($value) . "' and s.nStaffId !='1')
...
?>
<form name="frmMails" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>">
