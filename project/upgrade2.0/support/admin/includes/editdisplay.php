...
// +----------------------------------------------------------------------+
   // $addOredit = 'Add Display';
    ...
    if ($_GET["stylename"] != "") {
        ...
        $sql .=" where nCSSId ='".addslashes($var_id)."'";
        ...
        elseif ($_POST["postback"] == "A") {
            ...
            $sql="SELECT nCSSId FROM sptbl_css  WHERE   vCSSName  ='".addslashes($var_desc) . "'";
                      ...
                      $sql = "Insert into sptbl_css(nCSSId,vCSSName,vCSSURL,dDate";
                          $sql .= ") Values('','" . addslashes($var_desc). "','" . addslashes($file_name) . "',now())";
                          executeQuery($sql,$conn);
                          $var_insert_id = mysql_insert_id($conn);
                          //Insert the actionlog
						  if(logActivity()) {
                          $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_ADDITION . "','CSS','" . addslashes($var_insert_id) . "',now())";
                          executeQuery($sql,$conn);
						  }
                          $var_desc="";
        ...
        elseif ($_POST["postback"] == "D") {
                ...
                if (validateDeletion($var_id) == true and $var_id !="1") {
                        ...
                        //Insert the actionlog
						if(logActivity()) {
                        $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_DELETION . "','CSS','" . addslashes($var_id) . "',now())";
                        executeQuery($sql,$conn);
						}
                        $var_desc= "";
                        $var_url="";
           ...
           $sql="SELECT nCSSId  FROM sptbl_css  WHERE  vCSSName ='".addslashes($var_desc) . "'";
                   ...
                   if(mysql_num_rows($rs)>0){
                        ...
                        if (validateUpdation() == true and $dup_flag==0 and $errorcode=="") {
                          if($file_name !=""){
                            $file_name ="styles/".$file_name;
                            $seturlfld=" ,vCSSURL ='".addslashes($file_name)."' ";
                                //unlink the old file
                                 $sql="SELECT vCSSURL     FROM sptbl_css WHERE   nCSSId =$var_id";
                         $rs_oldurl = executeSelect($sql,$conn);
                                 $rowoldurl=mysql_fetch_array($rs_oldurl);
                             ...
                             $seturlfld=" ";
                                 ...
                                 $sql = "Update sptbl_css  set vCSSName   ='" . addslashes($var_desc) . "',
                                             dDate =now()";
                                ...
                                //Insert the actionlog
								if(logActivity()) {
									 $sql = "Insert into sptbl_actionlog(nALId,nStaffId,vAction,vArea,nRespId,dDate) Values('','$var_staffid','" . TEXT_UPDATION . "','CSS','" . addslashes($var_id) . "',now())";
									 executeQuery($sql,$conn);
								 }
...
?>
<form name="frmDisplays" method="POST" action="<?php echo($_SERVER['PHP_SELF']); ?>" enctype="multipart/form-data">
