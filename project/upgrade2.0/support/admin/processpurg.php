...
if (!is_writable("./purgedtickets/attachments")) {
   ...
   echo("<table><tr><td>admin/purgedtickets/attachments " . TEXT_NEED_EXECUTE . "</td></tr></table>");
	...
	$sql .= ($flag_where == true)?")":"";
	if($_GET["frm"] != "") {
		$sql .= ($flag_where == true)?( " AND (t.dLastAttempted >= DATE_FORMAT('" . addslashes($_GET["frm"]) . "','%Y-%m-%d %H:%i:%s')"):(" Where (t.dLastAttempted >= DATE_FORMAT('" . addslashes($_GET["frm"]) . "','%Y-%m-%d %H:%i:%s')");
		$flag_where=true;
		$flag_sub = true;
	}
	if($_GET["to"] != "") {
		$sql .= ($flag_where == true)?( " AND t.dLastAttempted <= DATE_FORMAT('" . addslashes($_GET["to"]) . "','%Y-%m-%d %H:%i:%s')"):(" Where (t.dLastAttempted <= DATE_FORMAT('" . addslashes($_GET["to"]) . "','%Y-%m-%d %H:%i:%s')");
		$flag_where=true;
		$flag_sub = true;
	}
	$sql .= ($flag_sub == true)?")":"";
