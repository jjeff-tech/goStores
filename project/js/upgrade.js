...
$("#jqNumYears").click(checkdomainavailability);
    ...
    $('#jqShowMessage').text('');
     $("#jqProductPayNow").validate({
                rules: {RegistrantFirstName: {required: true,maxlength: 30},
                    RegistrantEmailAddress: {required: true, email: true,maxlength: 80},
                    RegistrantLastName: {required: true,maxlength: 30} ,
                    RegistrantJobTitle: {required: true},
                    RegistrantOrganizationName: {required: true,maxlength: 100},
                    RegistrantAddress1: {required: true,maxlength: 41},
                    RegistrantAddress2: {required: false,maxlength: 41},
                    RegistrantCity: {required: true,maxlength: 30},
                    RegistrantPostalCode:{required: true,minlength: 2,maxlength: 10},
                    RegistrantFax: {required: false,maxlength: 17},
                    RegistrantPhone: {required: true,maxlength: 17},
                    ccno2:{required: true},
                    cvv2:{required: true}
                ...
                messages: {RegistrantFirstName: {required: "Please enter first name",maxlength: "Maximum 30 characters are allowed for first name field"},
                    RegistrantEmailAddress: {required: "Please enter email", email:"Please enter a valid email"}	,
                    RegistrantLastName: {required: "Please enter last name",maxlength: "Maximum 30 characters are allowed for last name field"},
                    RegistrantJobTitle: {required: "Please enter job title"},
                    RegistrantOrganizationName: {required: "Please enter organization name",maxlength: "Maximum 100 characters are allowed for organization field"},
                    RegistrantAddress1: {required: "Please enter address1",maxlength: "Maximum 41 characters are allowed for address1 field"},
                    RegistrantAddress2: {maxlength: "Maximum 41 characters are allowed for address2 field"},
                    RegistrantCity: {required: "Please enter city",maxlength: "Maximum 30 characters are allowed for city field"},
                    RegistrantPostalCode: {required: "Please enter zip/postal code",minlength: "Maximum 2 characters are needed for ZIP/Postal Code field",maxlength: "Maximum 10 characters are allowed for zip/postal code field"},
                    RegistrantFax: {maxlength: "Maximum 17 characters are allowed for fax field"},
                    RegistrantPhone: {required: "Please enter phone number",maxlength: "Maximum 17 characters are allowed for phone field"},
                    ccno2: {required: "Please enter card number"},
                    cvv2: {required: "Please enter cvv number"}
                }
            });

    $("#jqBackToPlan").click(function(){showPlansList('tab4')});
    $(".jqPlans").click(function(){updateSelectedPlan(this)});
    $(".jqBackToDomain").click(function(){showPlansList('tab1')});

    $("#li_tab3").find('a').hover(function(){ $("#li_tab3").find('a').css('background' , 'none'); });
...
{      
    ...
    var lookupsubdomain = $("#productLookUpId_subdomain").val();
...
if(currentPaymant == 'paypalpro' || currentPaymant == 'paypalflow' || currentPaymant == 'authorize' || currentPaymant == 'bluedog' || currentPaymant == 'stripe'){
    paymantFlage = 1;
}else{
     paymantFlage = 2;
}
        ...
        if(currentPaymant=='stripe')
        {
            ...
            //alert(SetUpgradeStripeToken)

            //alert(PublishKey);return false;

            //console.log(payNow);return false;

             urlData = urlData + "&fname="+$("#fname").val()+"&lname="+$("#lname").val()+"&add1="+$("#add1").val()+"&city="+$("#city").val()+"&state="+$("#RegistrantState").val()+"&country="+$("#country").val()+"&phone="+$("#phone").val()+"&zip="+$("#zip").val()+"&txtStoreName="+$("#txtStoreName").val()+"&email="+$("#email").val()+"&productId="+$("#productId").val()+"&ServiceAmount="+$("#jqFinalPrice").val()+"&serCat="+serCat+queryData;
             urlData = urlData + "&couponNo="+$("#couponNumber2").val();
             urlData = urlData + "&upgradeFlag="+$("#upgradeFlag").val()+"&productLookUpId="+$("#productLookUpId").val();
                ...
                data     : "price="+price+"&plan="+planName+"&key="+secretKey+"&email="+$("#email").val(),
                type     : "post",
...
return false;
 ...
 urlData = "expM="+$("#expM1"+currentID).val()+"&expY="+$("#expY1"+currentID).val()+"&cvv="+$("#cvv1"+currentID).val()+"&ccno="+$("#ccno1"+currentID).val()+"&productLookUpId="+lookupsubdomain;
 urlData = urlData + "&fname="+$("#fname").val()+"&lname="+$("#lname").val()+"&add1="+$("#add1").val()+"&city="+$("#city").val()+"&state="+$("#state").val()+"&country="+$("#country").val()+"&phone="+$("#phone").val()+"&zip="+$("#zip").val()+"&txtStoreName="+$("#txtStoreName").val()+"&email="+$("#email").val()+"&productId="+$("#productId").val()+"&ServiceAmount="+$("#jqFinalPrice").val()+"&serCat="+serCat+queryData;
 urlData = urlData + "&couponNo="+$("#couponNumber2").val();
        ...
        if($("#txtStoreName").val()=="")
            {
                ...
                $("#jqShowMessage").text("Please enter a subdomain before proceeding to the next step");
                return false;
            }
...
{ 
    //alert("Function cretaedomain in upgrade.js file");
    var ajaxSubmitOption 	= {
        url      : createAccount,
            ...
            else if(data.success)
            {
                if(flag==1)
                {
                    ...
                    $('.jqTldPrice').val(tldprice);
                     $('#spanjqTldPrice').text(parseFloat(tldprice).toFixed(2));
...
var lookupdomain = $("#productLookUpId_domain").val();
...
if(currentPaymant == 'paypalprodomain' || currentPaymant == 'paypalflowdomain' || currentPaymant == 'authorizedomain' || currentPaymant == 'bluedogdomain' || currentPaymant =='stripe'){
    paymantFlage = 1;
}else{
     paymantFlage = 2;
}
 ...
 urlData = queryData + "&expM="+$("#expM1"+currentID).val()+"&expY="+$("#expY1"+currentID).val()+"&cvv="+$("#cvv1"+currentID).val()+"&ccno="+$("#ccno1"+currentID).val()+"&productLookUpId="+lookupdomain;
...
//    $("#jqPriceTagArea").html(priceTag); // Price Tag Area
    $(".jqSubTotal").html(parseFloat(priceTag).toFixed(2));
   
    $(".jqTotalPurchaseVal").html(parseFloat(priceTag).toFixed(2));
    doCalculatediscount();
...
function removecouponcode(field){
    var couponNumber = $('#'+field).val();
    ...
    if(couponNumber!=''){
        $.ajax({
            ...
            if(obj.valid!=1){
                ...
                $('#'+field+'_err').html('<label class="sucess">'+'Coupon Code removed successfully'+'</label>');
                $('.jqDiscount').html(parseFloat(0).toFixed(2));
                $('#jqFinalPrice').val(total.toFixed(2));
                $('.jqTotalPurchaseVal').html(total.toFixed(2));
                $('#jqCpricemode').val('');
...
serviceDur = $("#product_bill").val() ;
    ...
    $.ajax(ajaxSubmitOption);
    var tldval = $('#jqTldPrice').val();
    $('#spanjqTldPrice').val(parseFloat(tldval).toFixed(2));
    return false;
}
